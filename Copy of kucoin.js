...
async function getPositions()
{
  try {
    ...
    {
      let decimals;
      if (avax.isNative(coins[i]))
        decimals = 18;
      else
        decimals = maps.decimalsMap.get(coins[i]);
 ...
 console.log("getPosition",coins[i],result);
      let pos = {
        ...
        amount: BigNumber(Math.floor(1000000*result.balance))
          .mult(BigNumber(10).pow(decimals)).div(1000000).toString(),
        decimals: decimals
      };
    ...
    throw new Error(e.message+" => kucoin.getPositions()");
  }
}
...
async function getOrderbook()
{
  try {
    ...
    let data = await kucoin.getPartOrderBook({amount:100,symbol:"AVAX-USDC"});
...
async function getOrders()
{
  try {
    ...
    let result = check(await kucoin.getOrders({type:"limit", status:"active"}));
...
console.log("supportedPair",sym0,sym1);
  if (avax.isNativeEquivalent(sym0) && avax.isStablecoin(sym1) ||
