...
async function checkTokenId(c,tid)
{
  ...
  {
    if (lowTickMap.get(parseInt(tickLower)) !== undefined)
      return false;
    ...
    console.log("Setting liquidity ",posInfo.liquidity, "tickLower",tickLower);
    return true;
  }
  return false;
}
...
async function getPositions(wname,walletAddress)
{
  ...
  liquidityMap.forEach((value,key) => {
      ...
      var newpos = [{ id: pid, symbol: 'ETH', decimals: 18, tickLower: tl, liquidity: liq, amount: vamt, quote: q, usd: vusd },
