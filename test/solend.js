...
function getPositions(list,mult)
{
  ...
  for (let i=0;i<list.length;i++)
  {
    ...
    if (mult == 1)
    {
      newpos = { symbol: list[i].symbol, amount: list[i].depositedAmount, usd: list[i].valueUSD };
    }
    else
    {
      newpos = { symbol: list[i].symbol, amount: -list[i].borrowedAmount, usd: -list[i].valueUSD };
    }
    pos.push(newpos);
  }
  return pos;
...
async function getPosition(walletAddress) {
  try {
    const solendUrl = 'https://api.solend.fi/v1/user-overview?wallet='+walletAddress;
