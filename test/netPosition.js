...
async function getPriceOfTokenAave(aaveOracleContract, token) {
  const tokenPrice = await aaveOracleContract.methods
    ...
    .catch(function (e) {
      console.log(e.message);
      throw new Error(e.message+" => getPriceOfTokenAave failed, likey because you are not using a token native to the Aave platform"
      );
    });
...
async function getPriceOfAllTokensAave(aaveOracleContract) {
  try {
    ...
    for (let i = 0; i < aaveTokenAddresses.length; i++) {
      ...
      tokenToPrice.push({ symbol: sym,token: aaveTokenAddresses[i], price: price });
    }
    return tokenToPrice;
  } catch (e) {
...
async function adjustDebt(debtTokenFrom, amount, tokenTo) {
  try {
    ...
    const amountOfTokensFromInWallet = await erc20.balanceOf(tokenFromContract,wallet.address); //get balance of tokens from in wallet
    ...
    const amountOfTokensToInWallet = await erc20.balanceOf(tokenToContract,wallet.address); //get balance of tokens to in wallet
    ...
    tokensToRepay = Math.floor(
      ...
      Math.min(await erc20.balanceOf(tokenToContract,wallet.address), tokensToRepay) //incase there are not enough tokens in the wallet due to slippage, i take the minimum of the two
    );
...
async function calculateNetPosition(wname,addr,mailResults)
{
  try 
  {
    ...
    if (parseInt(avVal) > spread)
    {
      ...
      if (mailResults)
        nodemailer.sendMail("SELL "+avTokens+" AVAX", 
          "You are LONG, SELL "+avTokens+" AVAX = "+avVal+". Net Value = "+netVal);
    }
    else if (parseInt(avVal) < -spread)
    {
      ...
      //console.log("USD=$", uVal);
      if (mailResults)
        nodemailer.sendMail("BUY "+avVal+" of AVAX", 
          "You are SHORT, BUY "+parseInt(avTokens)+" AVAX = "+avVal+". Net Value = "+netVal);
    }
    else
    {
      avTokens = addCommas(avTokens);
...
async function initNetPosition(wname,mailResults)
{
  ...
  await calculateNetPosition(wname,wallet.address,mailResults);
}
...
async function main() {
  try {
    ...
    wallet = wall.initWallet(wname);
    nodemailer.initEmailTo(wname);
    calculateNetPosition(wname,wallet.address,true);
...
module.exports = Object.assign({initNetPosition,calculateNetPosition,symbolMap,addressMap,decimalsMap,priceMap},nodemailer,wall,wallet)
