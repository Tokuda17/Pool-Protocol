...
async function cgQuote(cgid,retries=0,sleeptime=0) {
  try {
    ...
    if (retries == 0)
      throw Error(e.message," => cgQuote failed");
    else
    {
      retries--;
      if (sleeptime == 0)
...
function oneQuote(sym)
{
  ...
  const q = parseInt(BigNumber(maps.priceMap.get(sym)).mult(avaxPrice18).div(BigNumber(10).pow(18+9)).toString())/1000000000;
...
async function oneFastQuote(qaddr,ch="avax")
{
  try {
    ...
    //console.log("URL" , url);
    let allQuotes = await axios.get(url);
    console.log(allQuotes);
exit();
    let prices = allQuotes.data;
...
//    var usdcPrice18 = maps.priceMap.get(usdcAddr);
    let usdcPrice18;
    for (let addr in prices)
    {
      addr = addr.toLowerCase();
      if (usdFlag && qaddrFlag) break;
      if (addr == qaddr)
      {
        //maps.priceMap.set(addr.toLowerCase(), prices[addr]);
        qPrice = prices[addr];
        qaddrFlag = true;
      }
...
module.exports = Object.assign({cgQuote,oneQuote,oneQuoteFromAddress,oneLoadQuotes,oneFastQuote})
