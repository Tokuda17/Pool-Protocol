...
const web3 = web.web3;
const UNISWAP_POSITION_PATH="/Users/phdlance/Google Drive/My Documents/Finance/Crypto/DeFi Project/lance/uniswap";
const CACHE_POSITION_PATH="/Users/phdlance/Google Drive/My Documents/Finance/Crypto/DeFi Project/lance/cache";
const TAG_POSITION_PATH="/Users/phdlance/Google Drive/My Documents/Finance/Crypto/DeFi Project/lance/cache";
...
console.log("path=",path);
    ...
    fs.writeFileSync(path+"/"+id+".json", json, err => {
      if (err) {
        console.error(err);
...
function readTagId(tag,id,ch=false)
{
  try {
    ...
    if (ch)
      chain = ch;
    const path = CACHE_POSITION_PATH+"/"+chain+"/"+tag+"/"+id+".json";
...
//console.log("REMOVEUNISWAPID from cache",i);
    ...
    const path = UNISWAP_POSITION_PATH+"/"+chain+"/"+wname+"/"+i+".json";
    const e = fs.existsSync(path);
    if (e)
    {  
...
//    console.log("write Uniswap Id", wname, i, tid, tickLower, tickUpper, liquidity);
    ...
    fs.mkdirSync(path, { recursive: true });
    fs.writeFileSync(path+"/"+i+".json", json, err => {
      if (err) {
        console.error(err);
        throw new Error("Could write to "+wname+" "+i+" => fs.writeFile()");
      }
    });
  } catch (e) {
