...
async function getPositionsMulti(port,wallet) {
  try {
    ...
    //console.log("==================================================================");
    console.log("getPositions walletAddress=",wallet.walletAddress,"chain=",wallet.chain);
    pos=[];
    let price = maps.priceMap.get("ETH");
    //console.log("wallet.getPositions addessMap=",maps.addressMap);
    for (const[sym,a] of maps.addressMap) {
      ...
      {
        d = 18;
        bal = await getSpendableBalancee(walletAddress,wallet.chain);
      //console.log("getWallet3 XXXXXXXXXXXXXXXXXX  address=",a, " sym=", s);
      }
      else
      {
        c=await erc20e.getContract(a,wallet.chain);
        d=await erc20e.decimals(c,wallet.chain);
        bal=await erc20e.balanceOf(c,walletAddress,wallet.chain);
...
async function getPositions(walletAddress, addressMap, includelist=false, blocklist=false) {
  try {
    //console.log("==================================================================");
    console.log("getPositions walletAddress=",walletAddress,"chain=",web3.chain);
    pos=[];
    let price = maps.priceMap.get("ETH");
    ...
    for (const[s,a] of addressMap) {
      ...
      else if (web3.chain == 'poly')
      {
        ...
        {
          let vusd = parseInt(BigNumber(bal).mult(Math.floor(price * 1000000)).div(BigNumber(10).pow(18)).toString())/1000000;
          pos.push({id: "wallet", chain: web3.chain, symbol: "WETH", amount: bal, decimals: d, quote: price, usd: vusd});
        }
        else if (chain.isStablecoin(s))
        {
          let susd = parseInt(bal)/1000000;
    ...
    if (web3.chain == 'avax')
    {
      if (!blocklist.includes("AVAX"))
      {
        if (!includelist || includelist.includes("AVAX")) 
          pos.push({id: "wallet", chain: web3.chain, symbol: "AVAX", amount: native, decimals: 18});
      }
    }
    else if (web3.chain == 'op')
    {
...
console.log("wallet.init.3");
        ...
        web3.ethers[ch].wallet = new alchemy.Wallet(process.env.LANCE_PRIVATE_KEY,wallet);
...
module.exports = Object.assign({init,initPort,getPositions, getPositionsMulti,getBalance, getSpendableBalance, getSpendableBalancee,NATIVE_WALLET_RESERVE, MIN_WALLET_RESERVE})
