const gasPoly = require("./gasPoly2.js");
...
//console.log("h8 status = ",status);
          ...
          throw new Error("Transaction failed with status="+tx.status+" tx="+tx);
        }
...
console.log("alchemy gasPrice=",gasPrice.toString());
      ...
      let v = alchemy.Utils.parseEther(Math.floor(parseInt(BigNumber(data.tx.value)
        .div(BigNumber(10).pow(18-6)).toString()))/1000000+"");
      data.tx.value = v;
      console.log("value=",v);
      //console.log(data);
      console.log("data.tx=",data.tx);
console.log("1inche.swapWithQuote sending Transaction", data.tx, web3.chain);
