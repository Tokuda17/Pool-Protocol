...
let MAX_RETRIES=5;
const path = '/Users/phdlance/Google Drive/My Documents/Finance/Crypto/DeFi Project/lance/alpha/';
...
console.log("web3.chain",bankAddressMap.get(web3.chain));
  const bankContract = new web3.obj.eth.Contract(bankABI, 
...
async function bankWithdraw(wname, bankContract, id, walletAddress, retries =0) {
  try {
    const panContract = await getPanContract();
    ...
    if (retries < MAX_RETRIES && utils.shouldRetry(e.message))
    {
      ...
      return await bankWithdraw(wname,bankContract,id,walletAddress,retries);
    }
...
async function claimRewards(wname,walletAddress,rewards)
{
  ...
  for(let i=0;i<rewards.length;i++)
  {
    ...
    {
      await claimReward(bankContract, spellContract, rewards[i].poolId, walletAddress);
      claimed = true;
    }
  }
...
async function test(wname)
{
  const bankContract = await getBankContract();
  const spellContract = await getSpellContract();
  const data = await getData(spellContract,
...
function positionRemoved(wname, id)
{
  ...
  fs.writeFile(path+wname+"/"+id, "", err => {
    if (err) {
      console.error(err);
      throw new Error("Could not remove position "+wname+" "+id+" => positionRemoved()");
    }
  });
}
...
function getMinPositionId()
{
  if (web3.chain == "avax")
    ...
    throw new Error("getMinPositionId bad chain");
}
...
console.log ("compare owner getPositionIdsFromBlockchain", i, owner,pos.owner);
      if (pos.owner.toLowerCase() == owner.toLowerCase()  && BigNumber(pos.collateralSize).gt(0))
      {
        mypos.push(i);
...
async function approveLP(tokenA, tokenB, amt, walletAddress) {
  try {
    ...
    await erc20.approve(lpContract, walletAddress, bankAddressMap.get(web3.chain), amt);
    console.log("Success");
  } catch (e) {
    console.log(e.message);
...
async function claimReward(bankContract,spellContract,posId,walletAddress) { 
  try {
    ...
    const tx = await bankContract.methods.execute(
        posId,
        spellAddress,
        data)
...
async function getPendingRewards(wname, walletAddress)
{
  try {
    ...
    let rewardQuote;
    //rewardQuote = await quote.oneFastQuote(maps.addressMap.get("PNG"));
    rewardQuote = await quote.cgQuote('pangolin',2);
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      pendingReward = await getPendingReward(wchefContract,chefContract,ids[i],pos, rewardQuote);
...
async function getPendingReward(wchefContract, chefContract, poolId, pos,rquote)
{
  var pendingRewards = [];
  ...
  const pendingReward = parseInt(BigNumber(pos.collateralSize).mult(BigNumber(pInfo.accRewardPerShare).subtract(decodedCollateral.pngPerShare)).div(BigNumber(10).pow(36-10)).toString())/10000;
  console.log("PENDING REWARD",pendingReward);
  if (pendingReward > 0)
  {
    pendingRewards.push({poolId: poolId, reward: pendingReward, 
...
async function getUniswapPosition(pid,posInfo)
{
  ...
  console.log("lower",lower,"upper",upper,"liq",liq,"plower",plower,"pupper",pupper, "price",p);
  var pos = [{ id: pid, symbol: 'ETH', decimals: 18, amount: vamt },
...
async function getPositionsOp(owner) {
  try {
    ...
    const url = "https://api.homora.alphaventuredao.io/v2/10/uniswapv3/positions?userAddress="+owner;
    console.log("url",url);
...
async function getPositions(wname,owner)
{
  ...
  else if (web3.chain == "op")
  {
    ...
    throw new Error("unsupported chain "+web3.chain+" in alpha.getPositions()");
  return pos;
}
...
async function getPositionsAvax(wname, owner,retries=0) {
  try {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      let rquote;
      //rquote = await quote.oneFastQuote(maps.addressMap.get("PNG"));
      rquote = await quote.cgQuote('pangolin',2);
      const pendingReward = await getPendingReward(wchefContract,chefContract,ids[i],pos,rquote);
...
console.log("panResult=",panResult);
      ...
      let stable;
      if (isStablecoin(poolTokens[0].symbol))
        stable = 0;
      else
        stable = 1;
      nativeExposure = BigNumber(nativeExposure).add(poolTokens[stable].amount).toString();
    ...
    const result = {positions: mypos, rewards: pendingRewards, rewardsUsd: rewardsUsd, positionIds: ids, numPositions: ids.length, nativeExposure: nativeExposure, poolNativePrice: poolNativePrice};
    return result;
  } catch (e) {
    if (retries < MAX_RETRIES && utils.shouldRetry(e.message))
    {
    ...
    console.log(e.message);
    throw new Error(e.message+" retries="+retries+" => alpha.getPositions failed");
  }
}
