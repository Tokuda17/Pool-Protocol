...
function assertEthPair(sym0,sym1)
{
  ...
  if (isStablecoin(sym0) && isEthEquivalent(sym1) ||
...
async function getEthQuote()
{
  try {
    ...
    let qeth=false,qavax=false,qop=false,qpoly=false,qarb=false;
...
async function swap(port,fromSym,toSym,fromAmt,toAmt=false,ratio=false)
{
  try {
    failedChains.clear();
    while (true)
      ...
      if (s.quote)
      {
        ...
        if (toAmt)
        {
          ...
          if (isEthEquivalent(toSym))
            toAmt = BigNumber(Math.floor(toAmt*1000000)).mult(BigNumber(10).pow(18-6)).toString();
          else if (isStablecoin(toSym))
            toAmt = Math.floor(toAmt*1000000);
          else
            throw new Error("Unknown slippage check "+toSym+" => swap() failed");
          if (BigNumber(s.toAmt).lt(toAmt))
            return false;
          console.log("OPTIMISTIC check passed target toAmt=",toAmt,"quote=",s.toAmt);
        }
        try {
          console.log("SWAP.step3");
          unic.saveFile("trade","SWAP step 3 tries="+tries);
          unic.writeTagId("trade", Math.floor(Date.now()/1000)+".nonfusionQuote", 
            {
              fromSym: s.fromSym, 
              fromAmt: s.fromAmt, 
              toSym: s.toSym, 
...
console.log("Ready to swapWithQuote() s.chain=",s.chain);
            await inche.swapWithQuote(s.quote, s.quote.data.fromToken.address, s.fromAmt, wallet.address);
          }
          else
          {
            console.log("swap.step3b fusion",s);
            unic.writeTagId("fusion", Math.floor(Date.now()/1000)+".fusionQuote", 
              {
                fromSym: s.fromSym, 
                fromAmt: s.fromAmt, 
                toSym: s.toSym, 
              ...
              s.chain);
            let result = await fusion.swap(s.chain,s.fromSym,s.toSym,fromAmt,wallet.address);
            if (!result)
            {
        ...
        } catch (e)
        {
          ...
          if (f)
            failedChains.set(s.chain,f+1);
          else
            failedChains.set(s.chain,1);
          continue;
        }
...
async function adjustForGas(gas, sym, amount, ch, quote)
{
  ...
  if (ch == "poly")
  {
    ...
    cost = parseInt(BigNumber(gas).mult(gasPrice).div(BigNumber(10).pow(18-6)).toString())/1000000;
  }
  else if (ch == "arb")
  {
    ...
    let gasPrice = await alc.core.getGasPrice();
    gasPrice = gasPrice.toString();
    cost = parseInt(BigNumber(gas).mult(gasPrice).mult(Math.floor(quote*1000)).div(BigNumber(10).pow(18-9)).toString())*0.7/1000000;
  }
  else if (ch == "op")
  {
    ...
    let gasPrice = await alc.core.getGasPrice();
    gasPrice = gasPrice.toString();
    cost = parseInt(BigNumber(25000).mult(gasPrice).mult(Math.floor(quote*1000)).div(BigNumber(10).pow(18-9)).toString())/1000000;
...
console.log("amount=",amount,cost);
    amount = BigNumber(amount).minus(Math.floor(cost*1000000)).toString();
console.log("amount=",amount,cost);
  }
  else
  {
    amount = BigNumber(amount).minus(BigNumber(Math.floor(cost/quote*1000000000)).mult(BigNumber(10).pow(18-9))).toString();
  }
  console.log("adjustForGas",gas,sym,ch,quote, "cost=",cost,"newAmount=",amount);
  return amount;
}

const FUSION_SLIPPAGE = 0.007;

function hasChain(port,ch)
{
//console.log("hasChain port.wallets=",port.wallets,ch);
  for (let i=0;i<port.wallets.length;i++)
  {
//console.log("hasChain",port.wallets[i].chain,ch);
...
async function quoteEthSwap(port,sym0,sym1,amt0,opt=false,ratio=false)
{
...
console.log("port=",port);
    ...
    {
console.log("checking pair",ethPairs[i]);
      if (!hasChain(port,ethPairs[i].chain))
        continue;
console.log("pair ok");
...
//console.log("4",BigNumber(Math.floor(amt0*1000000)).toString());;
        fromAmt = BigNumber(Math.floor(amt0*1000000)).mult(BigNumber(10).pow(18-6)).toString();
...
//console.log("quoteEthSwap wamt=",wamt,ethPairs[i].chain);
      ...
      {
        try {
          quote = await fusion.getQuote(ethPairs[i].chain,fromSym,toSym,amt0);
        } catch (e) {
          console.log(e.message+" fusion.getQuote() failed");
          //throw new Error(e.message+" TEMPORARY ABORT fusion.getQuote() "+fromSym+" "+toSym+" "+amt0);
          continue;
        }
        toAmt = BigNumber(quote.toTokenAmount).mult((1-FUSION_SLIPPAGE)*1000000).div(1000000).toString();
        console.log("FUSION QUOTE for ch="+ethPairs[i].chain+" is "+toAmt);
        console.log("=======================================================");
        console.log("=======================================================");
        console.log("=======================================================");
        console.log("=======================================================");
      ...
      {
        console.log("6",fromSym,chain.getAddress(fromSym),toSym,chain.getAddress(toSym));;
        try {
          q = await inch.getQuote(chain.getAddress(fromSym),chain.getAddress(toSym),fromAmt,wallet.address,ethPairs[i].chain,opt,ratio);
        } catch (e) {
          console.log("quoteEthSwap error =",e.message,"continuing");
          continue;
        }
        console.log("7", q.quoteData.data.tx.gas, sym1, q.quoteData.data.toTokenAmount, ethPairs[i].chain, port.snapshot.quote);
        toAmt = await adjustForGas(q.quoteData.data.tx.gas, sym1, q.quoteData.data.toTokenAmount, ethPairs[i].chain, port.snapshot.quote);
        quote = q.quoteData;
      }
console.log("8");
      quotes.push(quote);
