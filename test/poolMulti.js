...
//console.log("Comparing MATIC=",c," with WALLET_MATIC_BUFFER");
    if (c < WALLET_MATIC_MIN)
    {
      ...
      body += await addPositionChanges(port,"poly",false,"managePoly.addMATIC");
...
console.log("positions=",positions," upos=",upos);
      ...
      body += await addPositionChanges(port,port.uniswapV3.chain,direction,"defundPosition");
      nodemailer.sendMail("Defunding position",body);
      return upos;
    }
    return false;
  } catch (e) {
    console.log(e.message," => defundPosition() failed");
...
console.log("addPositionChanges",wweth0,wweth1,newpos,ch);
      ...
      if (direction == "up" && wweth1 <= wweth0 || direction == "down" && wweth1 >= wweth0)
      {
        ...
        if (tries >= MAX_TRIES)
        {
          s = "tries="+tries+" wweth0="+wweth0+" wweth1="+wweth1+" direction="+direction+"\n check failed\n\n"+s;
          return s;
        }
        if (tries == 1)
        {
...
async function fundIncrease(port,tid,usd)
{
  try {
    ...
    let c = await univ3.getContract();
    await univ3.increaseLiquidity(port.uniswapV3.wname,c, port.uniswapV3.walletAddress, tid, port.snapshot.collateral-usd);
    let now = Math.floor(Date.now()/1000);
    ...
    body += await addPositionChanges(port,port.uniswapV3.chain,"down","increase liquidity");
    nodemailer.sendMail(subject,body);
...
async function fundMint(port)
{
  try {
    ...
    let apy =  Math.floor(((1+(pos.netVusd+pos.netSusd)/port.uniswapV3.pair.value) ** ((365*24*3600)/(pos.now-pos.starttime)) * 100-100)*10)/10;
    let body = "Profit="+profit+", APY="+apy+"%\n";
    body += await addPositionChanges(port,port.uniswapV3.chain,"down","mint");
    nodemailer.sendMail(subject,body);
...
console.log("pos=",newpos);
    if (parseFloat(newpos.netVusd) > port.snapshot.threshold)
    {
      ...
      if (wc > parseFloat(newpos.netVamt))
      {
        ...
        await sell(port,false,parseFloat(newpos.netVusd)/port.snapshot.threshold);
        return;
      }
      else
      {
        newpos.notes = "Non optimistic, need WETH, adjust.defundPosition() weth="+wc;
        unic.saveObj("trade",newpos);
...
console.log("u=",u,-parseFloat(newpos.netVusd));
      if (u > -parseFloat(newpos.netVusd))
      {
        ...
        await buy(port, -newpos.netVusd, false, -parseFloat(newpos.netVusd)/port.snapshot.threshold);
        return;
      }
      else
      {
        newpos.notes = "Non optimistic, need USDC, adjust.defundPosition() usdc="+u;
        unic.saveObj("trade",newpos);
    ...
    if (!fp)
    {
      if (parseFloat(newpos.netVusd) > port.snapshot.threshold * port.uniswapV3.pair.optRatio)
      {
        let wc = utils.checkWallet(newpos.positions,"WETH",port.uniswapV3.chain);
        if (wc > parseFloat(newpos.netVamt))
        {
        ...
        {
          newpos.notes = "Optimistic, need WETH, adjust.defundPosition() weth="+wc;
          unic.saveObj("trade",newpos);
          await init(port,port.uniswapV3.chain,"defundPosition"); 
          let tid = await defundPosition(port,true);
          if (tid)
      ...
      else if (parseFloat(newpos.netVusd) < -port.snapshot.threshold * port.uniswapV3.pair.optRatio)
      {
console.log("need to sell usdc to buy weth");
        let u = utils.checkWallet(newpos.positions,"USDC",port.uniswapV3.chain);
console.log("u=",u,-parseFloat(newpos.netVusd));
        ...
        {
          newpos.notes = "Optimistic, need USDC, adjust.defundPosition() usdc="+u;
          unic.saveObj("trade",newpos);
          await init(port,port.uniswapV3.chain,"defundPosition"); 
          let tid = await defundPosition(port,true);
          if (tid)
            ...
            throw new Error("Not enough USD "+u+" => adjust() failed");
        }
      }
    }
...
async function checkWeth(newpos,ch,reset=true)
{
  ...
  if (wcache)
  {
    ...
    if (wcache.direction)
      direction = wcache.direction;
    
    let sym = "WETH";
    let wweth = utils.checkWallet(newpos.positions,sym,ch);
    if (direction == "up" && wweth <= cweth ||
        !direction && wweth != cweth ||
        direction == "down" && wweth >= cweth)
    {
      await utils.sleep(12);
      throw new Error("Bad WETH wallet value in checkWeth() cweth="
        +cweth+" newpos.wweth="+wweth+" direction="+direction+" chain="+ch);
    }
    if (direction && reset)
    {
      let w = {wweth: wweth};
