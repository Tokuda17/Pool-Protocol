...
//console.log("Comparing MATIC=",c," with WALLET_MATIC_BUFFER");
    if (c < WALLET_MATIC_MIN)
    {
      ...
      body += await addPositionChangesMulti(port,pos,"poly",false,"managePoly.addMATIC");
...
//console.log("Comparing MATIC=",c," with WALLET_MATIC_BUFFER");
    if (c < WALLET_MATIC_MIN)
    {
      ...
      let body = "MATIC total increasing\n";
      body += await addPositionChanges(wname,walletAddress,pos,false,"managePoly.addMATIC");
      nodemailer.sendMail("Swapping USDC for MATIC",body);
      return true;
    }
    // defund positions that are out of range
...
console.log("manageOp");
    ...
    if (c < WALLET_ETH_MIN && wc > WALLET_ETH_BUFFER)
    {
      ...
      body += await addPositionChangesMulti(port,pos,"op","down","manageOp.addETH");
...
console.log("manageOp");
    ...
    if (c < WALLET_ETH_MIN && wc > WALLET_ETH_BUFFER)
    {
      ...
      let body = "ETH total increasing\n";
      body += await addPositionChanges(wname,walletAddress,pos,"down","manageOp.addETH");
      nodemailer.sendMail("Swapping WETH for ETH",body);
      return true;
    }
    console.log("before defundPos");
...
console.log("positions=",positions," upos=",upos);
      ...
      let direction = false;
      if (pos.quote < epos.pupper)
        direction = "up";
      let body = "Defunding="+upos+"\n";
      body += await addPositionChangesMulti(port,pos,port.uniswapV3.chain,direction,"defundPositionMulti");
...
console.log("positions=",positions," upos=",upos);
      ...
      if (pos.quote < epos.pupper)
        direction = "up";
      let body = "Defunding="+upos+"\n";
      body += await addPositionChanges(wname,walletAddress,pos,direction,"defundPosition");
      nodemailer.sendMail("Defunding position",body);
      return upos;
    }
    return false;
...
console.log("addPositionChanges",wweth1,newpos);
      ...
      trace = trace+"d";
      if (direction == "up" && wweth1 <= wweth0 || direction == "down" && wweth1 >= wweth0)
      {
        trace = trace+"e";
        tries++;
        ...
        if (tries >= MAX_TRIES)
        {
          ...
          throw new Error("addPositionChangeMulti callfrom="+callfrom+" wweth0="+wweth0+" wweth1="+wweth1+" trace="+trace+" tries="+tries+"\n\n"+poss);
        }
        nodemailer.sendMail(
          "WETH wallet error: "+callfrom+" ch="+ch,
          "Detected bad WETH wweth0="+wweth0+" wweth1="+wweth1+" dir="+direction+" trace="+trace+" tries="+tries+"\n\n"+poss);
    ...
    console.log(e.message);
    throw new Error(e.message+" trace="+trace+" => addPositionChangesMulti() failed");
  }
}

async function addPositionChanges(wname,walletAddress,pos,direction,callfrom)
{
  let trace = "a";
  try {
...
console.log("addPositionChanges",wweth1,newpos);
      ...
      if (direction == "up" && wweth1 <= wweth0 || direction == "down" && wweth1 >= wweth0)
      {
        ...
        if (tries >= MAX_TRIES)
        {
          ...
          throw new Error("addPositionChange callfrom="+callfrom+" wweth0="+wweth0+" wweth1="+wweth1+" trace="+trace+" tries="+tries+"\n\n"+poss);
        }
        nodemailer.sendMail(
...
async function fundIncreaseMulti(port,pos,tid,usd)
{
  try {
    ...
    let c = await univ3.getContract();
    await univ3.increaseLiquidity(port.uniswapV3.wname,c, port.uniswapV3.walletAddress, tid, SPAN_VALUE-usd);
    let now = Math.floor(Date.now()/1000);
    ...
    body += await addPositionChangesMulti(port,pos,port.uniswapV3.chain,"down","increase liquidity");
    nodemailer.sendMail(subject,body);
...
async function fundIncrease(wname,walletAddress,pos,tid,usd)
{
  try {
    ...
    let body = "Profit="+profit+", APY="+apy+"%\n";
    body += "Increasing by "+(SPAN_VALUE-usd)+"\n\n";
    body += await addPositionChanges(wname,walletAddress,pos,"down","increase liquidity");
    nodemailer.sendMail(subject,body);
    //unic.saveFile("update",wname,"Increasing liquidity");
  } catch (e) {
    console.log(e.message);
...
async function fundMintMulti(port,pos)
{
  try {
    ...
    let apy =  Math.floor(((1+(pos.netVusd+pos.netSusd)/SPAN_VALUE) ** ((365*24*3600)/(pos.now-pos.starttime)) * 100-100)*10)/10;
    let body = "Profit="+profit+", APY="+apy+"%\n";
    body += await addPositionChangesMulti(port,pos,port.uniswapV3.chain,"down","mint");
    nodemailer.sendMail(subject,body);
...
//console.log("balance vusd < -thresh",newpos.netVusd < -TRADE_THRESHOLD);
      ...
      {
        if (await manageOpMulti(port,newpos))
          return;
      }
      else if (port.wallets[i].chain == "poly")
      {
        if (await managePolyMulti(port,newpos))
          return;
      }
    }
...
console.log("u=",u,-parseFloat(newpos.netVusd));
      ...
      else
      {
        ...
        if (tid)
          ...
          throw new Error("Not enough USDC "+u+" => adjust() failed");
      }
    }
    else
    {
...
console.log("END OF ADJUST");
    ...
    nodemailer.sendMail("adjustMulti() failed", e.message+" => adjustMulti() failed");
...
async function adjust(wname, walletAddress)
{
  try {
    ...
    if (wcache && checkWallet(newpos.positions,"WETH") != wcache.wweth)
    {
      ...
      throw new Error("Bad WETH wallet value in adjust() wcache.wweth="
        +wcache.wweth+" newpos.wweth="+checkWallet(newpos.positions,"WETH"));
    }
...
console.log("balance vusd < -thresh",newpos.netVusd < -TRADE_THRESHOLD);
    if (web3.chain == "op")
    {
      if (await manageOp(wname,walletAddress,newpos))
        return;
    }
    else if (web3.chain == "poly")
    {
      if (await managePoly(wname,walletAddress,newpos))
        return;
    }
console.log("checking if vusd > TRADE_THRESHOLD");
    if (parseFloat(newpos.netVusd) > TRADE_THRESHOLD)
    {
      ...
      else
      {
        ...
        if (tid)
          ...
          throw new Error("Not enough WETH "+wc+" => adjust() failed");
      }
    }
    else if (parseFloat(newpos.netVusd) < -TRADE_THRESHOLD)
    {
...
console.log("gp1a", maps.priceMap);
    ...
    for (let i=0;i<port.wallets.length;i++)
    {
      ...
      npos = await wall.getPositions(port.wallets[i].walletAddress, maps.addressMap);
    ...
    port = await initMulti(port,port.uniswapV3.chain,"port.uniswapV3.chain",web3.chain); 
...
async function assertWeth(newpos,direction,ch)
{
  ...
  if (wcache)
  {
    ...
    if (direction == ">" && wweth <= wcache ||
        direction == "=" && wweth != wcache ||
        direction == "<" && wweth >= wcache)
    {
      await utils.sleep(12);
      throw new Error("Bad WETH wallet value in assertWeth() wcache="
        +wcache+" newpos.wweth="+wweth+" direction="+direction+" chain="+ch);
    }
  }
}
...
async function calculateMulti(port)
{
  ...
  {  
    ...
    console.log(e.message);
    if (port.email)
      nodemailer.sendMail("calculateMulti() failed", e.message);
...
async function calculate(wname,walletAddress,email=true)
{
  ...
  {  
    ...
    {
      await utils.sleep(12);
      throw new Error("Bad WETH wallet value in calculate() wcache.wweth="
        +wcache.wweth+" newpos.wweth="+checkWallet(newpos.positions,"WETH"));
    }
    compoundPosition(newpos);
    newpos= prunePositions(newpos);
