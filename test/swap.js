...
async function getQuote(fromToken,toToken,fromAmount,walletAddress,override=false,debug="")
{
  try {
    ...
    if (!override)
    {
      ...
      inchResult = await inch.getQuote(fromToken,toToken,fromAmount,walletAddress);
...
async function findSwap(
  ...
  maxSlip,
  debug = "")  // slippage increases linearly from min to max over seconds
{
...
console.log("toSymbol1",toSymbol, "fromSymbol1", fromSymbol);
  ...
  while (time < startTime + seconds) {
    try {
      ...
      q = await getQuote(fromToken, toToken,fromAmount, walletAddress,false,debug);
...
// xxx this section only works for trading USDC/AVAX, need to change if we add more pairs
            ...
            if (avax.isNativeEquivalent(fromSymbol))
            {
              ...
              //toSymbol = "USDT";
              toSymbol = "USDC";
              // xxx no longer workslet avaxPrice = quote.oneQuote("WAVAX");
              let avaxPrice = maps.priceMap.get("WAVAX");  // should work because oneLoadQuotes was called previously
...
async function swap(fromToken, toToken, fromAmount, walletAddress, override=false,debug="")
{
  try {
    await kucoin.init();
    await kucoin.getOrderbook();
    const q = await getQuote(fromToken, toToken,fromAmount, walletAddress,override,debug);
    let useKuCoin = false;
    ...
    let avaxAmount;
    let limitPrice;
    if (q.kucoin !== false && (q.kucoin.toAmount > q.inch.toAmount || override))
    {
      ...
      if (override)
      {
        ...
        const amtScaled =
          parseInt(BigNumber(fromAmount2).div(BigNumber(10).pow(decimals-6)).toString())/1000000;
        //let q2 = await kucoin.getQuote("AVAX-USDT",fromSymbol,amtScaled,override);
        let q2 = await kucoin.getQuote("AVAX-USDC",fromSymbol,amtScaled,override);
