...
async function mint(contract,walletAddress,lowerNut,value,tries=0,gp=false)
{
  try {
    let calls = [];
...
async function increaseLiquidity(wname,c,walletAddress,tokenid,value,tries=0,gp=false)
{
  try {
    let calls=[];
...
console.log("Amount=",test,test0,test1);
  ...
  console.log("increaseLiquidity params=",params);
    ...
    {   
      let gp = utils.parseGas(e.message);
      gp = Math.floor((1+0.5*tries) * gp);
      await increaseLiquidity(wname,c,walletAddress,tokenid,value,tries,gp);
      return; 
    }
    console.log(e.message);
...
function getInitPosition(tl,liq)
{
  //SPAN = 180;
  //HALF_SPAN=90;
  ...
  var vamt = parseInt(BigNumber(liq).mult(Math.floor((Math.sqrt(pupper) - Math.sqrt(p))/Math.sqrt(p)/Math.sqrt(pupper)*1000000)).toString());
console.log("vamt",vamt);
  let vamt18 = BigNumber(Math.floor(vamt)).toString();
console.log("vamt18",vamt18);
  vamt = parseInt(BigNumber(vamt).div(BigNumber(10).pow(10)).toString())/100000000;
console.log("vamt",vamt);
  let vusd = parseInt(BigNumber(vamt18).div(BigNumber(10).pow(12)).toString())/1000000*p;
console.log("vusd",vusd);
  var samt = parseInt(BigNumber(liq).mult(Math.floor((Math.sqrt(p) - Math.sqrt(plower))*1000000)).toString())/1000000;
console.log("samt",samt);
  samt = Math.floor(samt / (10 ** 6));
console.log("samt",samt);
  let susd = parseInt(samt)/1000000;
console.log("susd",susd);
  console.log("getInitPosition","lower",tl,"liq",liq,"plower",plower,"pupper",pupper, "price",p,"susd",susd,"vamt",vamt);
  return {iVamt: vamt, iSusd: susd};
}
...
//console.log("f0",f0,"f1",f1);
        var newpos = [
          { id: pid, chain: web3.chain, symbol: 'ETH', decimals: 18, inRange: inRange, tickLower: tl, plower: plower, pupper: pupper, liquidity: liq, amount: vamt, quote: q, usd: vusd },
          { id: pid, chain: web3.chain, symbol: 'USDC', decimals: 6, inRange: inRange, tickLower: tl, plower: plower, pupper: pupper, liquidity: liq, amount: samt, quote: 1, usd: susd },
          ];
        ...
        if (liq > 0)
        {
          ...
          newpos = [
            { id: "fees-"+pid, chain: web3.chain, symbol: 'ETH', amount: f0, usd: f0 * q },
            { id: "fees-"+pid, chain: web3.chain, symbol: 'USDC', amount: f1, usd: f1 }
          ];
          pos = pos.concat(newpos);
        }
