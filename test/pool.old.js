...
const OPTIMISTIC_TRADE_THRESHOLD = 0.40; // percent 
...
async function calculateNetPositions(wname,owner) {
  try {
    ...
    //console.log("get alpha Position",alphaPos);
    const alphaRewards = alphaPos.rewards;
    //console.log("ALPHA REWARDS:", alphaRewards);
    const rewardsUsd = alphaPos.rewardsUsd;
    ...
    let netpos = {
      ...
      nativeExposure: alphaPos.nativeExposure,
...
console.log("NETVAL: ", netPosition.netVal,init.netValue);
    ...
    if (avVal > spread)
    {
      ...
      subject = "SELLING "+printAvTokens+" AVAX";
    ...
    else if (avVal < -spread)
    {
      ...
      subject = "BUYING "+printMavVal+" of AVAX";
      body += "Profit="+printProfit+", APY="+apy+"%\n";
...
async function calculateAndAdjust(wname,addr,mailResults=true,init)
{
  try {
    ...
    const dt=new Date(init.timestamp * 1000).toLocaleString("en-US",
      {timeZone: "America/Chicago"});
    if (avVal > spread * OPTIMISTIC_TRADE_THRESHOLD && wname == "lance" ||
        avVal > spread)
    {
      subject = "SELLING "+printAvTokens+" AVAX";
      body += "Profit="+printProfit+", APY="+apy+"%\n";
    ...
    if (wname == "lance")
      await checkAndReloadWallet(addr,maps.addressMap.get("USDC.e"));
  } catch (e) {
    console.log(e.message);
    nodemailer.sendMail("calculateAndAdjust() failed", e.message + " => calculateAndAdjust failed");
...
function mailPosition(netPosition,init)
{
  ...
  let netVal = netPosition.netVal;
  let profit = netPosition.netVal - init.netValue;
console.log("NETPOS: ", netPosition.netVal, init.netValue);
  const now = Date.now()/1000;
  const elapsed = now - init.timestamp;
  var apy = ((1+profit/init.collateral) ** ((365*24*3600)/elapsed)) *100-100;
  profit = Math.floor(profit*100)/100;
...
console.log("step4");
    ...
    let spread=parseInt(netPos.nativeExposure)/1000000/2.75*getTradeThreshold(wname)/100*100000000;
...
function getDebtTradeParams(tradeType,params={})
{
  if (tradeType == "default")
  {
    if (SWAP_ATTEMPTS == 1)
    {
      params = {minThresh: 300, maxThresh: 500, seconds: 60};
    }
    else
    {
      params = {minThresh: 500, maxThresh: 600, seconds: 60};
    }
  }
  else if (tradeType == "profit")
  {
    ...
    else if (SWAP_ATTEMPTS <= 1)
    {
      params = {minThresh: 0, maxThresh: 0, seconds: 40};
    }
    else if (SWAP_ATTEMPTS == 2)
    {
      params = {minThresh: 0, maxThresh: 0, seconds: 40};
    }
    else if (SWAP_ATTEMPTS == 3)
    {
      params = {minThresh: 0, maxThresh: 0, seconds: 40};
    }
    else if (SWAP_ATTEMPTS == 4)
    {
      params = {minThresh: 0, maxThresh: 0, seconds: 40};
    }
  }
...
console.log("tokenFromBal", tokenFromBal);
    ...
    console.log ("adjustDebt params for getSwapQuote", minThresh, maxThresh, seconds);
    ...
    if (BigNumber(tokenFromBal).gt(minimum))
    {
      ...
      {
        try {
          var status = await swap.findSwap(
            tokenFrom,
            tokenTo,
            tokenFromBal,
            seconds,
          ...
          if (e.message.search("Request failed with status code 400")>=0)
            throw new Error(e.message+" tokenFromBal="+tokenFromBal+" => adjustDebt()");
          else
            throw new Error(e.message);
        }
        if (!status)
          throw new Error("getSwapQuote failed to execute on SWAP_ATTEMPTS="+SWAP_ATTEMPTS+" tradeType="+tradeType);
      }
