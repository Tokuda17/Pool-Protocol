...
function findOrder(orders,orderHash)
{
  ...
  {
    if (orderHash == orders.items[i].orderHash)
      return orders.items[i];
  }
...
async function printOrder(orderHash,walletAddress)
{
  ...
  let orders = await sdk.getOrdersByMaker({page: 1, limit: 50, address: walletAddress});
...
async function monitorOrder(ch,orderHash,amount, walletAddress)
{
  ...
  {
    let orders = await sdk.getOrdersByMaker({page: 1, limit: 5, address: walletAddress});
    console.log("orders=",orders);
...
console.log("swap",ch,fromSym,toSym,amount,walletAddress);
  ...
  const amt = BigNumber(Math.floor(amount*1000000)).mult(BigNumber(10).pow(decimals-6)).toString();
  let sdk = web.getFusion(ch);
  let params = {
