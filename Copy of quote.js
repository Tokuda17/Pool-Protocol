...
//exit();
  ...
  {
    try {
      const baseUrl = 'https://pro-api.coingecko.com/api/v3/simple/price?x_cg_pro_api_key=CG-ygVcBbpKkz5ihGh5bkSTok2D';
      const url = baseUrl+"&ids="+cgids+"&vs_currencies=usd&include_24hr_change=true";
      console.log("Entering quote",url);
      const quote = await axios.get(url);
      if (!quote.data)
      {
        throw new Error("Coingecko did not return data for quote of "+cgids);
      }
...
function getCgValues(ch=false)
{
  ...
  {
    throw new Error("Unknown chain "+ch+" in getCgValues()");
  }
}

function getCgListFromTokens(tokens)
{
  let list = tokens[0].cgid;
...
async function cgQuote(cgid,retries=0,sleeptime=0) {
  ...
  while (retries >= 0)
  {
    try {
      const baseUrl = 'https://pro-api.coingecko.com/api/v3/simple/price?x_cg_pro_api_key=CG-ygVcBbpKkz5ihGh5bkSTok2D';
      const url = baseUrl+"&ids="+cgid+"&vs_currencies=usd&include_24hr_change=true";
      console.log("Entering quote",url);
      const quote = await axios.get(url);
      console.log("quote returned ");
...
async function loadCgQuotes(tokens,ch="avax")
{
  try {
    let cgids = getCgListFromTokens(tokens);
    let qs = await cgQuotes(cgids,retries=2);
 console.log("getCgQuotes=",qs);
    for (let i=0;i<tokens.length;i++)
    {
      maps.priceMap.set(tokens[i].symbol,qs[tokens[i].cgid].usd)
    }
    console.log("Price Map=", maps.priceMap);
  } catch (e) {
    console.log(e.message);
    throw Error(e.message," => getCgQuotes failed");
  }
}
...
async function oneFastQuote(qaddr,ch="avax")
{
  try {
    ...
    let quotes;
    const url = "https://api.1inch.dev/price/v1.1/" + id;
    quotes = await axios.get(url,config);
    quotes = quotes.data;
    console.log("quotes" ,quotes);
    //quotes = JSON.parse(qdata);
    console.log("finding quote for qaddr=",qaddr);
    console.log(quotes[qaddr.toLowerCase()]);
    var usdFlag = false;
    var qaddrFlag = false;
    let usdcAddr = "0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E";
    if (id ==1)
...
module.exports = Object.assign({cgQuote,cgQuotes,oneLoadQuotes,oneFastQuote,loadCgQuotes})
