...
async function borrow(walletAddress,sym,amt)
{
  ...
  const onBehalfOf = walletAddress;
    ...
    if (retries < MAX_RETRIES)
    {
      if (shouldRetry(e.message))
      {
        retries++;
        await utils.sleep(3 ** retries);
        return await borrow(aavePoolContract, debtToken, amount, walletAddress, retries);
      }
    }
    throw Error(e+" debtToken="+debtToken+" amount="+amount+" => borrow() failed");
  }
}

async function repay(walletAddress,sym,amt,amount=false)
{
  const poolContract = await getPoolContract();
  ...
  if (!amount)
    amount = BigNumber(Math.floor(amt * 1000000)).mult(BigNumber(10).pow(decimals-6)).toString();
...
) 
{
  try {
    ...
    throw Error(e+" token="+token+" amount="+amount+" => repay() failed");
  }
}

async function deposit(walletAddress,sym,amt)
{
  const poolContract = await getPoolContract();
  const token = maps.addressMap.get(sym);
  const c = await erc20.getContract(token);
...
async function withdraw(walletAddress,sym,amt,amount=false)
{
  const poolContract = await getPoolContract();
  const token = maps.addressMap.get(sym);
  const c = await erc20.getContract(token);
...
console.log("withdrawInternal",token,amount,walletAddress);
  await aavePoolContract.methods
  ...
  try {
    ...
    for (let i = 0; i < debtTokenAddresses.length; i++) {
      ...
      if (debt > 0) {
        ...
        let p = maps.priceMap.get(sym.toUpperCase());
        if (!isNaN(p))
        {
          let amt = parseInt(BigNumber(debtOfToken.amount).div(BigNumber(10).pow(d-6)).toString())/1000000;
          let usd = p * amt;
          debtOfToken.amt = amt;
          debtOfToken.usd = usd;
        }
...
collateralMap.set("USDC",0.825);

function getRatio(pos,actions=false)
{
  let collateral = 0;
  let debt = 0;
  ...
  try {
    ...
    for (let i = 0; i < aTokenAddresses.length; i++) {
      ...
      if (balance > 0) {
        ...
        if (!isNaN(p))
        {
          let amt = parseInt(BigNumber(balanceOfToken.amount).div(BigNumber(10).pow(d-6)).toString())/1000000;
          let usd = p * amt;
          balanceOfToken.amt = amt;
          balanceOfToken.usd = usd;
        }
