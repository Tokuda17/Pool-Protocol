...
*/
  ...
  try {
    ...
    const amount = parseInt(BigNumber(walletPng.amount).div(BigNumber(10).pow(walletPng.decimals)));
...
function expandPoolPositions(pos, poolAddresses)
{
  ...
  for (let i=0;i<pos.length;i++)
  {
    ...
    {
      tokens = pan.getPoolTokens(pos.symbol,maps.addressMap(pos.symbol), pos.amount);
...
async function calculateNetPositions(wname,owner) {
  ...
  try {
    ...
    var pos3 = await wall.getPositions(owner,maps.addressMap,false,false);
    trace += "d";
...
//console.log("pos3=",pos3);
    ...
    wallNativeExposure = getNativeExposure(['AVAX-USDC','AVAX-USDC.e','AVAX-USDT'],pos4);
    //console.log("=====================\n");
    //console.log(pos4);
    pos = pos.concat(pos4);
    if (wname == "lance" )
    ...
    trace += "h";
    let exchanged = false;
    // xxx avoiding 403 and 400 errors
    exchanged = await exchangeRewards(wname,owner,walletPng);
    trace += "i";
    if (exchanged)
      return calculateNetPositions(wname,owner);
    let usd = 0; 
    let native = 0; 
    for (let i = 0; i < pos.length; i++) {
      //console.log("i=",i, pos[i]);
...
console.log("netpos in calculateNetPositions",netpos);
    ...
    throw new Error(e.message+" => calculateNetPositions failed trace="+trace);
  }
}

async function calculateRebalanceAndAdjust(wname,addr,mailResults=true,init)
{
  try {
    ...
    if (BigNumber(lp).gt(0))
    {
      ...
      await leverDeposit(addr,poolStableToken,borrowVariableToken, 
        factoryContract, bankContract,panPoolAddress,lp);
    ...
    if (numPositions == 0 || debtRatio < DEBT_RATIO_LOWER || debtRatio > DEBT_RATIO_UPPER) 
    {
      if (wname != "lance")
      {
        console.log("ADJUSTING POSITION, debt ratio is", debtRatio);
        console.log("POSITIONS = ", netPosition.alphaNumPositions);
        rebalanceMsg = "REBALANCE Required: debt ratio is " + debtRatio + ", calling adjustPosition.\n";
        console.log("******************************************");
        console.log("3a. Rebalance required, calling adjustPosition");
        console.log("******************************************");
        await adjustPosition(wname, addr,"USDC","WAVAX","USDC.e","AVAX",netPosition);
        netPosition = await calculateNetPosition(wname,addr);
      }
      else
      {
        rebalanceMsg = "REBALANCE Required: debt ratio is " + debtRatio + ", NOT calling adjustPostion for lance's portfolio.\n";
      }
    }
    else
    {
...
//      }
    ...
    nodemailer.sendMail("calculateRebalanceAndAdjust() failed", e.message + " => calculateRebalanceAndAdjust failed");
...
async function calculateAndAdjust(wname,addr,mailResults=true,init)
{
  try {
    ...
    console.log("*************************************************************");
      ...
      nodemailer.setSubjectStartOption("EMERGENCY: Call Lance immediately - ");
      let mtime = Math.floor(Date.now()/1000);
      subject = "EMERGENCY: Call Lance immediately. "+printAvTokens+" AVAX " + mtime;
      body += "Profit="+printProfit+", APY="+apy+"%\n";
    ...
    else if (avVal > spread * OPTIMISTIC_TRADE_THRESHOLD && wname == "lance" ||
        avVal > spread)
    {
      let mtime = Math.floor(Date.now()/1000);
      subject = "SELLING "+printAvTokens+" AVAX " + mtime;
    ...
    nodemailer.init("lance");
    nodemailer.setSubjectStartOption("Pool Protocol: [lance] ");
...
console.log("init: ",init.netValue);
  ...
  let subject="";
  let body="";
  subject+=Math.floor(profit)+"("+Math.floor(avVal)+") "+apy+"%";
...
async function calculateNetPosition(wname,walletAddress,mailFlag=false,init=null)
{
  let trace="TRACE:";
  try
  { 
...
*/
    ...
    avaxUSDPrice = await quote.cgQuote("avalanche-2",2);
    trace += "c";
...
console.log("Step5");
    ...
    console.log(e.message);
    nodemailer.sendMail("calculateNetPosition() failed", e.message+" trace="+trace);
    throw new Error(e.message+" => calculateNetPosition failed trace="+trace);
  }
}
...
// await adjustDebt(wname,"WAVAX", avTokens, "USDC", addr, "default");
// await adjustDebt(wname,"USDC", -avVal, "WAVAX", addr, "default");
async function adjustDebt(wname,debtTokenSym, amount, tokenToSym, walletAddress, tradeType) {
  try {
    console.log("adjusting debt",wname,debtTokenSym,amount,tokenToSym,walletAddress,tradeType);
...
console.log(" POS1 tokenFromBal", tokenFromBal);
    ...
    let borrowAmount = BigNumber(amount)
      .minus(tokenFromBal)
      .minus(
          await aave.convertFromTokenToToken(tokenTo, tokenFrom, tokenToBal, walletAddress)
      ).toString();
...
console.log("availableDebtToRepay", availableDebtToRepay);
    if (!borrowMoney) throw new Error("Not enough collateral in Aave to borrow",borrowAmount,tokenFrom);
...
console.log("tokenFromBal", tokenFromBal);
    ...
    console.log ("adjustDebt params for findSwap", minThresh, maxThresh, seconds);
...
console.log ("CALLING SWAP for profit",tokenFrom,tokenTo,amount);
        try {
          ...
          if (e.message.search("Request failed with status code 400")>=0)
            throw new Error(e.message+" tokenFromBal="+tokenFromBal+" => swap.findSwap()");
          else
            throw new Error(e.message+" => swap.findSwap()");
        }
        if (!status)
          throw new Error("swap.findSwap failed to execute on SWAP_ATTEMPTS="+SWAP_ATTEMPTS+" tradeType="+tradeType);
      }
      else if (tradeType == "default")
      {
console.log ("CALLING SWAP for forced trade",tokenFrom,tokenTo,amount);
        DEFAULT_SWAP_ATTEMPTS++;
        if (wname == "lance")
        {
          let override = false;
          if (DEFAULT_SWAP_ATTEMPTS > 2)
            override = true;
          await swap.swap(
            tokenFrom,
            tokenTo,
        ...
        throw new Error("unrecognized tradeType => adjustDebt()");
    }
...
*/
    ...
    throw new Error(e.message + " adjustDebt failed SWAP_ATTEMPTS="+SWAP_ATTEMPTS+" DEFAULT_SWAP_ATTEMPTS="+DEFAULT_SWAP_ATTEMPTS);
  }
}
...
console.log("variableDebtToken",variableDebtToken);
    const variableDebtTokenContract = await erc20.getContract( variableDebtToken);
    ...
    for(let i=0;i<netPosition.alphaPositionIds.length;i++)
    {
      ...
      await alpha.bankWithdraw(wname, bankContract, netPosition.alphaPositionIds[i], walletAddress);
    }

    //Gets the total collateral in Aave
    const collateral = BigNumber(Math.floor(netPosition.collateral * 100000000)).toString();
    console.log("collateral", collateral);

    //Gets the debt ratio in Aave
    // xxx could get this from the result of calculateNetPosition, already being returned
    const debtRatio = await aave.getDebtRatio(borrowPoolContract, walletAddress);
    ...
    const stableReserve = BigNumber(usdWalletReserve).div(BigNumber(10).pow(12)).div(2).toString();
    const variableReserve = BigNumber(wall.NATIVE_WALLET_RESERVE).div(2).toString();
    ...
    idealStableBorrowAmount = BigNumber(idealStableBorrowAmount).minus(stableReserve).toString();
    idealVariableBorrowAmount = BigNumber(idealVariableBorrowAmount).add(variableReserve).toString();
    console.log( "idealStableBorrowAmount",
      idealStableBorrowAmount, 
...
console.log("borrowStableTokenSym", borrowStableTokenSym);
    let actualStableBorrowAmount = 
      BigNumber(getSymbolFromArray(borrowStableTokenSym,netPosition.aavePositions).amount).mult(-1).toString();
console.log("borrowVariableTokenSym", borrowVariableTokenSym);
    let actualVariableBorrowAmount = 
      BigNumber(getSymbolFromArray(borrowVariableTokenSym,netPosition.aavePositions).amount).mult(-1).toString();
    ...
    let stableDiff = BigNumber(idealStableBorrowAmount).minus(actualStableBorrowAmount).toString();
    let variableDiff = BigNumber(idealVariableBorrowAmount).minus(actualVariableBorrowAmount).toString();
...
console.log("get contract panPoolAddress", panPoolAddress);
    ...
    {
      if (BigNumber(stableDiff).lt(0) && checkThreshold(stableDiff,idealStableBorrowAmount,BORROW_THRESHOLD))
      {
...
*/
      ...
      if (BigNumber(variableDiff).lt(0) && checkThreshold(variableDiff,idealVariableBorrowAmount,BORROW_THRESHOLD))
      {
...
*/
      ...
      if (BigNumber(stableDiff).gt(0) && checkThreshold(stableDiff,idealStableBorrowAmount,BORROW_THRESHOLD))
      {
        console.log("BORROW stable coins from aave", stableDiff);
        await aave.borrow( borrowPoolContract, stableDebtToken, stableDiff, walletAddress);
      }
      //console.log("variableDebtToken", variableDebtToken, "borrowVariableToken",borrowVariableToken);
      if (BigNumber(variableDiff).gt(0) && checkThreshold(variableDiff,idealVariableBorrowAmount,BORROW_THRESHOLD))
      {
        console.log("BORROW variable coins from aave", variableDiff);
        await aave.borrow( borrowPoolContract, variableDebtToken, variableDiff, walletAddress);
      }

      await checkAndReloadWallet(walletAddress,poolVariableToken);

      if (poolStableTokenSym != borrowStableTokenSym)
        await convertTokens(walletAddress,borrowStableTokenContract, borrowStableToken,poolStableToken);
      if (poolVariableTokenSym != borrowVariableTokenSym)
        await convertTokens(walletAddress,borrowVariableTokenContract, borrowVariableToken,poolVariableToken);
      await balanceTokens(walletAddress, 
        poolStableTokenSym, poolStableTokenContract, poolStableToken,
        poolVariableTokenSym, poolVariableTokenContract, poolVariableToken);
    ...
    leverDeposit(walletAddress,poolStableToken,borrowVariableToken, factoryContract, bankContract,panPoolAddress,lp);
