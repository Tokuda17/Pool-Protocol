...
function checkAssets(port)
{
  const vsym = port.tj.pair.vsym;
  const ssym = port.tj.pair.ssym;
  const buffer = 1.05;
  let value = port.tj.pair.value;
  let half = value/2*buffer;
  if (port.snapshot.profit > 0)
    value += port.snapshot.profit;

  let vusd = getIdUsd(port,"wallet",vsym);
  let susd = getIdUsd(port,"wallet",ssym);

  if (vusd >= half && susd >= half)
    return true;
  if (susd < half)
  {
    const asusd = getIdUsd(port,"aave-avalanche",ssym);
...
async function checkTick(port,tick)
{
  ...
  const reb = port.tj.pair.rebalance;
  lowerThresh += reb;
  upperThresh -= reb;
...
async function mailReminder(action)
{
console.log("mailReminder",action);
    nodemailer.init("default");
    //nodemailer.init("lance");
    nodemailer.setSubjectStartOption("IMPORTANT: Action "+action);
    body = "Call Lance if you get four of these in a row! (5 min apart)";
...
console.log("getPositions=",npos);
    pos = pos.concat(npos);
    npos = await aave.getPositions(port);
    pos = pos.concat(npos);
...
console.log("profit=",profit);
    ...
    let tick = port.tj.pair.tick;
    let spanOffset = false;
    if (port.tj.pair.ids)
    {
      spanOffset = tick - port.tj.pair.ids[Math.floor(port.tj.pair.bins/2)];
console.log("spanOffset=",tick,port.tj.pair.ids[Math.floor(port.tj.pair.bins/2)],spanOffset);
    }
    let newpos = {
