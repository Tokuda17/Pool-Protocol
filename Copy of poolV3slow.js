/*
This version works for Uniswap V3.  It uses calculateNetPosition for each check to see if 
adjust should be called.
...
console.log("manageOp");
    ...
    if (c < WALLET_ETH_MIN && wc > WALLET_ETH_BUFFER)
    {
      ...
      let r = await inch.swap(maps.addressMap.get("WETH"),chain.getAddress("ETH"),amt,w.walletAddress);
      if (!r)
...
console.log("SELLING vamt=",vamt);
    ...
    let r = await multi.swap(port, vsym, ssym, vamt,samt, ratio);
    if (!r) return false;
    let ch = r.chain;
    let f = r.fusion;
...
console.log("BUYING vusd", vusd);
    let r = await multi.swap(port, ssym, vsym, samt, vamt,ratio);
    if (!r) return false;
    let ch = r.chain;
    let f = r.fusion;
    let now = Math.floor(Date.now()/1000);
