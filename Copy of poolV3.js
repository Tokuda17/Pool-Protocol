...
async function init(port,ch=false,callfrom=false)
{
  ...
  await univ3.init(port.uniswapV3.pair.span,port.uniswapV3.pair.spacing,port.uniswapV3.pair.poolFee);
  if (port.email)
  {
...
async function initOld(port,ch=false,callfrom=false)
{
  console.log("init ch=",ch);
  if (!ch)
...
console.log("tokenAddresses=",tokenAddresses,web3.chain);
  ...
  else if (web3.chain == "op")
  {
    ...
    tokenAddresses.push(OP_ADDRESS);
  }
  ...
  console.log("returning from init web3.chain=",web3.chain,callfrom);
  return port;
}
...
console.log("manageOp",port.walletAddress);
    ...
    let c = await wall.getBalance(port.uniswapV3.walletAddress);
console.log("c=",c);
    c = parseInt(BigNumber(c).div(BigNumber(10).pow(18-6)).toString())/1000000;
console.log("c2=",c);
...
{
  try {   
    let vsym = port.uniswapV3.pair.vsym;
    let ssym = port.uniswapV3.pair.ssym;
    let pos = port.snapshot;
...
console.log("SELLING vamt=",vamt);
    ...
    let r = await inch.swap(vsym, ssym, vamt);
    if (!r) return false;
    let ch = chain.getChain();
    let f = r.fusion;
    let now = Math.floor(Date.now()/1000);
    let profit = Math.floor((pos.netVusd+pos.netSusd)*100)/100;
    let subject = "Profit="+profit+" sell($"+Math.floor(vamt*maps.priceMap.get(vsym))+") "+opt+ch+" f="+f+" "+now;
...
async function buy(port,amt=false,slippage=false,ratio=false)
{
  try {   
    ...
    let samt = vusd/maps.priceMap.get(ssym);
    if (amt)
    {
      samt = amt;
    }
    ...
    if (slippage !== false)
    {
      vamt = vusd / maps.priceMap.get(vsym) * (1-slippage);
      opt = "OPTIMISTIC ";
    }
console.log("BUYING vusd", vusd, vsym,samt);
    let r = await inch.swap(ssym, vsym, samt);
    if (!r) return false;
    let ch = chain.getChain();
    let f = r.fusion;
    let now = Math.floor(Date.now()/1000);
...
async function fundIncrease(port,tid,usd)
{
  try {
    ...
    let c = await univ3.getContract();
    await univ3.increaseLiquidity(port.uniswapV3.wname,c, port.uniswapV3.walletAddress, tid, port.snapshot.collateral-usd,vsym,ssym);
    let now = Math.floor(Date.now()/1000);
...
async function fundMint(port)
{
  try {
    ...
    let c = await univ3.getContract(); 
    await univ3.mint(c, port.uniswapV3.walletAddress, lowerNut, port.snapshot.collateral,
        port.uniswapV3.pair.vsym,port.uniswapV3.pair.ssym);
    let now = Math.floor(Date.now()/1000); 
...
console.log("createPosition");
    ...
    position = findInRange(pos,port.uniswapV3.pair.ssym);
    if (position)  
    {
      let tid = position.id;
console.log("createPostion found ", tid);
      ...
      {
console.log("createPosition().fundIncrease()",port.snapshot.positions);
        await init(port,port.uniswapV3.chain,"fundIncrease");
        await fundIncrease(port,tid,0);
        return true;
      }
      else
      {
    ...
    {
console.log("createPosition().fundMint()",port.snapshot.positions);
...
console.log("fundPosition");
    let position = findInRange(pos,port.uniswapV3.pair.ssym);
    if (position)  
    {
      let tid = position.id;
...
console.log("port=",port);
  ...
  { 
    let wc = utils.checkWallet(newpos.positions,vsym,port.uniswapV3.chain);
...
console.log("need to sell usdc to buy weth",newpos.positions);
    let u = utils.checkWallet(newpos.positions,ssym,port.uniswapV3.chain);
console.log("u=",u,-parseFloat(newpos.netVusd));
    if (u > -parseFloat(newpos.netVusd)/maps.priceMap.get(ssym))
    {
...
async function rebalance(port,action,tick)
{
  let newpos = port.snapshot;
  let vsym = port.uniswapV3.pair.vsym;
  let ssym = port.uniswapV3.pair.ssym;
  let value = port.uniswapV3.pair.value;
  let ethPrice = await univ3.getEthPrice();
  let poolRatio = univ3.getTickPrice(tick);
  if (action == "sellOp")
  {
    let opPrice = ethPrice/poolRatio;
    let amt = value/opPrice/2;
    let wc = utils.checkWallet(newpos.positions,vsym,port.uniswapV3.chain);
    if (wc > amt)
    {
      newpos.notes = "Non optimistic adjust.sell() with ratio "+parseFloat(newpos.netVusd)/port.snapshot.threshold;
      unic.saveObj("trade",newpos);
      console.log("Not optimistic sell");
      let s = await sell(port,amt);
      return s;
    }
    return false;
  }
  else if (action == "buyOp")
  {
console.log("need to sell usdc to buy weth",newpos.positions);
    let u = utils.checkWallet(newpos.positions,ssym,port.uniswapV3.chain);
console.log("u=",u,-parseFloat(newpos.netVusd));
    let amt = value/ethPrice/2;
console.log("amt to buy with ETH=",amt);
    if (u > amt)
    {
      newpos.notes = "Non optimistic, adjust.buy() with ratio = "+(-parseFloat(newpos.netVusd)/port.snapshot.threshold);
      unic.saveObj("trade",newpos);
      console.log("Not optimistic buy");
      let s = await buy(port,amt);
      return s;
    }
    return false;
  }
...
{
  let lower = getTickLower(port.snapshot.positions);
  let halfSpan = port.uniswapV3.pair.span/2 * port.uniswapV3.pair.spacing;
  //console.log("halfSpan=",halfSpan);
  let half = lower + halfSpan;
  //console.log("half=",half);
  let lowerThresh = half - port.uniswapV3.pair.spanThresh;
  let upperThresh = half + port.uniswapV3.pair.spanThresh;
  console.log("lowerThresh=",lowerThresh,"upperThresh=",upperThresh);
...
//console.log("balance vusd < -thresh",newpos.netVusd < -port.snapshot.threshold);
    ...
    if (!state)
    {
      let position = findNonzeroLiquidity(newpos);
      if (position)
      {
        let profit = Math.floor(newpos.profit * 100)/100;
        setState("funded",{profit: profit});
        return false;
      }
    ...
    if (state == "funded")
    {
      const TIMEOUT = 300;
      const SLEEP = 1;
      const minProfit = port.uniswapV3.pair.value/2 * 0.004;
      const start = parseInt(Date.now()/1000);
      let now = parseInt(Date.now()/1000);
      let pc = await univ3.getContractForPool();
      let profit = getState("profit") + minProfit;
      if (port.snapshot.profit < profit || port.snapshot.profit < 0)
      {
        return false;
      }
      ...
      {
        let tick = await univ3.getPoolTick(pc);
        ...
        if (action == "sellOp" || action == "buyOp")
        {
          let r = await rebalance(port,action,tick);
          if (r)
          {
            setState("rebalanced");
console.log("r=true");
...
console.log("gp1y", q);
    ...
    let startSusd = startSamt * maps.priceMap.get(ssym);
...
console.log("web3=",web3);
    ...
    npos = await univ3.getPositions(port.uniswapV3.wname,port.uniswapV3.walletAddress,
        port.uniswapV3.pair.vsym,port.uniswapV3.pair.ssym);
console.log("returning from univ3.getPositions()",npos);
    let ivamt=0;
    let isamt=0;
    let isusd=0;
    let lVamt=0;
    ...
    if (npos && npos[0])
    {
      ...
      if (i < npos.length)
      {
        ...
        let {iVamt,iSamt} =  univ3.getInitPosition(tickLower,liq,vsym,ssym);
        ivamt = iVamt; 
        isamt = iSamt; 
        isusd = iSamt * maps.priceMap.get(ssym);
console.log("ivamt=",iVamt,"isamt=",iSamt);
        if (npos[i].symbol==ssym)
        {
          lVamt = npos[i+1].usd/maps.priceMap.get(vsym);
          lSamt = npos[i].usd/maps.priceMap.get(ssym);
        }
        else
        {
          lVamt = npos[i].usd/maps.priceMap.get(vsym);
          lSamt = npos[i+1].usd/maps.priceMap.get(ssym);
        }
        liqValue = lVamt*maps.priceMap.get(vsym) + lSamt*maps.priceMap.get(ssym);
      }
    }
    let initLiqValue = isamt*maps.priceMap.get(ssym) + ivamt*maps.priceMap.get(vsym);
    ...
    for(let i=0;i<pos.length;i++)
    {
      ...
      if (pos[i].symbol == ssym)
      {
        //console.log("stablecoin");
        susd += pos[i].usd;
      }
      else if (pos[i].symbol == vsym)
      {
    ...
    let profit = vusd-startVamt*maps.priceMap.get(vsym)+susd-startSusd;
    ...
    let newpos = {
      ...
      state: getState(),
      profit: profit,
      netVusd: vusd-startVamt*maps.priceMap.get(vsym),
      netVamt: vusd/maps.priceMap.get(vsym)-startVamt,
      netSusd: susd-startSusd,
      netSamt: (susd-startSusd)/maps.priceMap.get(ssym),
      vprice: maps.priceMap.get(vsym),
      sprice: maps.priceMap.get(ssym),
      iVamt: ivamt,
      iVusd: ivamt*maps.priceMap.get(vsym),
      iSamt: isamt,
      iSusd: isusd,
      lVamt: lVamt,
      lVusd: lVamt * maps.priceMap.get(vsym),
      lSamt: lSamt,
      lSusd: lSamt * maps.priceMap.get(ssym),
      ...
      vusd: vusd, 
      vamt: vusd/maps.priceMap.get(vsym), 
      samt: susd/maps.priceMap.get(ssym),
      susd: susd,
      quote: q, 
