const uniABI = require("./ABI/UniswapV3.json");
var BigNumber = require('big-number');
const utils = require("./utils.js");
const wall = require("./wallet.js");
const unic = require("./unicache.js");
const gas = require("./gasPoly2.js");
const gasOp = require("./gasOp.js");
const unifees = require("./unifees.js");
const chain = require("./chain.js");
const web = require("./web3.js");
const web3 = web.web3;
const erc = require("./erc20.js");
const quote = require("./quote.js");
const maps = require("./maps.js");
poolInterface = require('@uniswap/v3-core/artifacts/contracts/interfaces/IUniswapV3Pool.sol/IUniswapV3Pool.json');
quoterInterface = require('@uniswap/v3-periphery/artifacts/contracts/lens/Quoter.sol/Quoter.json');

const NF_POSITION_MANAGER_ADDRESS = "0xC36442b4a4522E871399CD717aBDD847Ab11FE88";
const ETH_ADDRESS = "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee";
const SUBGRAPH_URL = 'http://api.thegraph.com/subgraphs/name/uniswap/uniswap-v3';

const MAX_ETH_IN = "10000000000000000000000000";
const MAX_USD_IN = "1000000000000";
let POOL_FEE;

let TICK_SPACING;
let SPAN;
let HALF_SPAN;

let lowTickMap = new Map();
let liquidityMap = new Map();
let positionMap = new Map();

function setPosition(tid,i)
{
  positionMap.set(parseInt(tid),parseInt(i));
}

function setLowTick(lowerNut,tid)
{
  lowTickMap.set(parseInt(lowerNut),parseInt(tid));
}

function setLiquidity(lowerNut,liquidity)
{
  //console.log("SETLIQUIDITY",lowerNut,liquidity);
  liquidityMap.set(parseInt(lowerNut),liquidity);
}

function lookupLiquidity(lowerNut)
{
  let v = liquidityMap.get(parseInt(lowerNut));
  if (v !== undefined)
    return v;
  else 
    return v;
}

function lookupLowTick(lowerNut)
{
  let v = lowTickMap.get(parseInt(lowerNut));
  if (v !== undefined)
    return parseInt(v);
  else 
    return v;
}

function lookupPosition(tid)
{
  console.log("positionMap=",positionMap);
  let v = positionMap.get(parseInt(tid));
  if (v !== undefined)
    return parseInt(v);
  else 
    return v;
}

async function checkTokenId(wname,i,c,tid)
{
  console.log("Checking tid=",tid);
  setPosition(tid,i);
  let posInfo = unic.readUniswapId(wname,i);
  if (!posInfo)
  {
    console.log("Pos info not found");
    posInfo = await positions(c,tid); 
    unic.writeUniswapId(wname,i,tid,posInfo.tickLower,posInfo.tickUpper,posInfo.liquidity,posInfo.fee);
  }
  else
  {
    console.log("reading posInfo from cache for",i);
  }
  console.log("fee", posInfo," i=",i, parseInt(posInfo.fee),POOL_FEE);
  if (parseInt(posInfo.fee) != POOL_FEE)
  {
    console.log("fee mismatch for",i,tid);
    return false;
  }
  console.log(tid,"tickLower",posInfo.tickLower,"tickUpper",posInfo.tickUpper,"SPAN", SPAN);
  if ((parseInt(posInfo.tickLower) + SPAN == parseInt(posInfo.tickUpper)))
  {
    console.log("\n**** STAGE 1 *****\n");
    if (lookupLowTick(posInfo.tickLower) !== undefined)
    {
      console.log("tickLowerMap=",lowTickMap);
      console.log("\n**** EXIT EXIT EXIT "+lookupLowTick(posInfo.tickLower)+" ****\n");
      return false;
    }
    console.log("\n********\nFOUND tid=", tid, posInfo.tickLower);
    setLowTick(posInfo.tickLower,tid);
    setLiquidity(posInfo.tickLower,posInfo.liquidity);
    console.log("Setting liquidity ",posInfo.liquidity, "tickLower",posInfo.tickLower);
    return true;
  }
  else
  {
    console.log("\n**** NO MATCH *****\n");
  }
  return false;
}

async function getTokenIds(wname,c,walletAddress)
{
  console.log("getTokenIds()", wname, walletAddress);
  try {
    lowTickMap = new Map();
    liquidityMap = new Map();
    positionMap = new Map(); // maps from Uniswap tokenId to Uniswap position number (consecutive)
    for(i=0;;i++)
    {
      try {
        let tid = unic.checkUniswapId(wname,i);
console.log("id=",i,tid);
        if (!tid)
        {
          tid = await c.methods
            .tokenOfOwnerByIndex(walletAddress,i)
            .call();
        }
        if (!(await checkTokenId(wname,i,c,tid)))
        {
          continue;
        }
      } catch(e) {
        if (e.message.search("index out of bounds")>=0)
        {
          console.log("Exiting getTokenIds i=",i);
          break;
        }
        console.log(e.message);
        throw new Error(e.message+" => tokenOfOwnerByIndex() failed");
      }
    }
  console.log("liquidityMap=",liquidityMap);
  } catch (e) {
    console.log(e.message+" => getTokenIds() failed");
    throw new Error(e.message+" => getTokenIds() failed");
  }
}

function getTickPrice(ticks)
{
  console.log("getTickPrice",ticks);
  if (web3.chain == "poly")
    ticks = -ticks;
  let p = 1.0001 ** ticks;
  console.log("PRICE=",p);
  return p;
} 

function getTickFromPrice(price)
{
  let tick = Math.round(Math.log(price)/Math.log(1.0001));
  if (web3.chain == "poly")
    tick = -tick;
  return tick;
}

async function getPoolContract(pool) {
  try {
    const contract = await new web3.obj.eth.Contract(poolInterface.abi, pool);
    return contract;
  } catch (e) {
      console.log(e.message+" getPoolContract failed");
      throw Error(e.message+" => getPoolContract failed");
  }
}

async function getContract() {
  try {
    const contract = await new web3.obj.eth.Contract(uniABI, NF_POSITION_MANAGER_ADDRESS);
    return contract;
  } catch (e) {
      console.log(e.message+" getContract failed");
      throw Error(e.message+" => getContract failed");
  }
}

async function positions(contract,tokenid) {
  try {
    //console.log("POSITIONS", tokenid);
    //console.log("CONTRACT", contract);
    //console.log("METHODS", contract.methods);
    const info = await contract.methods
      .positions(tokenid)
      .call()
      .catch(function (e) {
        console.log(e.message);
        throw new Error(e.message+" => positions() failed");
      });
console.log("positions", tokenid,info);
    return info;
  } catch (e) {
      console.log(e.message+" positions failed");
      throw Error(e.message+" => positions failed");
  }
}

async function position(contract,tokenid,q)
{
  posInfo = await positions(contract,tokenid);
  const lower = posInfo.tickLower;
  const upper = posInfo.tickUpper;
  const liq = posInfo.liquidity;
  let plower = getTickPrice(lower);
  let pupper = getTickPrice(upper);
console.log("POSITION plower=",plower,"pupper=",pupper);
  if (plower > pupper)
  {
    let ptmp = plower;
    plower = pupper;
    pupper = plower;
  }
  if (q < plower)
    q = plower;
  else if (q > pupper)
    q = pupper;
  // need to update this to use 1inch quotes
  let vamt = liq * (Math.sqrt(pupper) - Math.sqrt(q))/Math.sqrt(q)/Math.sqrt(pupper);
  vamt = BigNumber(Math.floor(vamt)).mult(1000000).toString();
//console.log("q=",q,"plower=",plower,"liq",liq);
  let samt = liq * (Math.sqrt(q) - Math.sqrt(plower));
//console.log("samt=",samt);
  samt = Math.floor(samt / (10 ** 6));
//  console.log("lower",lower,"upper",upper,"liq",liq,"plower",plower,"pupper",pupper, "price",q);
  var pos = [{ symbol: 'ETH', decimals: 18, amount: vamt, tickLower: lower, liquidity: liq},
             { symbol: 'USDC', decimals: 6, amount: samt, tickLower: lower, liquidity: liq}];
  return pos;
}

const univ3sdk = require('@uniswap/v3-sdk');
const unicore = require('@uniswap/sdk-core');


const FACTORY_ADDRESS="0x1F98431c8aD98523631AE4a59f267346ea31F984";

let WETH_TOKEN;
let OP_TOKEN;
let USDC_TOKEN;

function init(span,spacing,poolFee)
{
  console.log("univ3.init()",span,spacing,poolFee);
  TICK_SPACING = spacing;
  SPAN = span * spacing;
  POOL_FEE = poolFee;
  HALF_SPAN = SPAN/2;
  console.log("univ3.init()",web3.chain);
  try {
    WETH_TOKEN = new unicore.Token(
      chain.chainId(),
      maps.addressMap.get("WETH"),
      18,
      'WETH',
      'Wrapped Ether'
    );
  
    USDC_TOKEN = new unicore.Token(
      chain.chainId(),
      maps.addressMap.get("USDC"),
      6,
      'USDC',
      'USD Coin'
    );
  
    OP_TOKEN = new unicore.Token(
      chain.chainId(),
      maps.addressMap.get("OP"),
      18,
      'OP',
      'Optimism'
    );
console.log("WETH_TOKEN",WETH_TOKEN,OP_TOKEN);
  } catch (e) {
    console.log(e.message+" => univ3.init() failed");
    throw new Error(e.message+" => univ3.init() failed");
  }
  //console.log("returning from univ3.init()");
}

// xxx pool is hard coded here
function getPool(tokenA,tokenB,fee)
{
  console.log("getPool() tokenA",tokenA);
  const poolAddress = univ3sdk.computePoolAddress({
    factoryAddress: FACTORY_ADDRESS,
    tokenA: tokenA,
    tokenB: tokenB,
    fee: fee });
  console.log("poolAddress=",poolAddress);
  return poolAddress;
}

async function feeGrowthGlobal0X128(c)
{
  const tu = await c.methods
    .feeGrowthGlobal0X128()
    .call()
    .catch(function (e) {
      console.log(e.message);
      throw new Error(e.message+" => feeGrowthGlobal0X128() failed");
    });
  return tu;
}

async function feeGrowthGlobal1X128(c)
{
  const tu = await c.methods
    .feeGrowthGlobal1X128()
    .call()
    .catch(function (e) {
      console.log(e.message);
      throw new Error(e.message+" => feeGrowthGlobal1X128() failed");
    });
  return tu;
}

async function slot0(c)
{
//console.log("CONTRACT for SLOT0",c);
  const s0 = await c.methods
    .slot0()
    .call()
    .catch(function (e) {
      console.log(e.message);
      throw new Error(e.message+" => s0() failed");
    });
  return s0;
}

async function ticks(c,tick)
{
//console.log("CONTRACT for TICKS",c);
  const t0 = await c.methods
    .ticks(tick)
    //.ticks("-201680")
    .call()
    .catch(function (e) {
      console.log(e.message);
      throw new Error(e.message+" => tickUpper() failed");
    });
  return t0;
}

async function tickSpacing(c)
{
  const ts = await c.methods
    .tickSpacing()
    .call()
    .catch(function (e) {
      console.log(e.message);
      throw new Error(e.message+" => ts() failed");
    });
  return ts;
}

async function liquidity(c)
{
  const liq = await c.methods
    .liquidity()
    .call()
    .catch(function (e) {
      console.log(e.message);
      throw new Error(e.message+" => s0() failed");
    });
  return liq;
}

function calculateAmounts(tick,lower,upper,value)
{
console.log("calculateAmounts tick=",tick,"lower=",lower,"upper=",upper,"value=",value);
  if (tick < lower) 
    tick = lower;
  else if (tick > upper)
    tick = upper;
  let p = getTickPrice(tick);
  let plow = getTickPrice(lower);
  let pup = getTickPrice(upper);
console.log("price=",p,"low=",plow,"pup=",pup);
  if (plow > pup)
  {
    let ptmp = plow;
    plow = pup;
    pup = ptmp;
  }
console.log("price=",p,"low=",plow,"pup=",pup);
  let v = (Math.sqrt(pup) - Math.sqrt(p))/Math.sqrt(p)/Math.sqrt(pup);
  let s = (Math.sqrt(p) - Math.sqrt(plow))/Math.sqrt(p)/Math.sqrt(plow);
console.log("v=",v,"s=",s);
  let vr = v/(s+v);
  let sr = s/(s+v);
//console.log("vr=",vr,"sr=",sr);
  return {vr, sr};
}

async function refundETH(contract,walletAddress)
{
  try {
    let multiParams = {
      from: walletAddress,
      value: 0,
      gas: 800000,
      nonce: await web3.obj.eth.getTransactionCount(walletAddress)
    };
    if (web3.chain == "poly")
      multiParams.maxPriorityFeePerGas = await gasOp.getGas();

    console.log("multiParams",multiParams);
    let tx = await contract.methods
      .refundETH()
      .send(multiParams);
    console.log("tx=",tx);
  } catch (e) {
    console.log(e.message);
    throw new Error(e.message+" => refundETH() failed");
  }
}

async function confirmTransaction(hash)
{     
  console.log("Confirming transaction hash=",hash);
  const start = parseInt(Date.now()/1000);
  let now = parseInt(Date.now()/1000);
  const TIMEOUT = 300;
  const SLEEP = 1; 
  let status = false;
  while (start + TIMEOUT > now)
  {
      
    status = await web3.obj.eth.getTransactionReceipt(hash);
    console.log("status=",status,"hash=",hash);
    if (status !== null)
    {
      return status;
    }
    await utils.sleep(SLEEP); 
    now = parseInt(Date.now()/1000);
  }
  throw new Error("Transaction not confirmed in 1inch.confirmTransaction()");
} 

async function getCurrentPrice()
{
  let price = await getPoolPrice(WETH_TOKEN,OP_TOKEN,POOL_FEE);
  return price;
}

async function getEthPrice()
{
  let price = await getPoolPrice(WETH_TOKEN,USDC_TOKEN,500);
  price=price * 1000000000000;
  return price;
}

async function getContractForPool()
{
  try {
    let p = await getPool(WETH_TOKEN,OP_TOKEN,POOL_FEE);
    console.log("Pool=",p);
    let pc = await getPoolContract(p);
    return pc;
  } catch (e) {
    console.log(e.message);
    throw new Error(e.message+" => getPoolContractFromTokens failed");
  }
  
}

async function getPoolTick(pc)
{
  try {
    let s0 = await slot0(pc);
    let tick = parseInt(s0[1]);
    return tick;
  } catch (e) {
    console.log(e.message);
    throw new Error(e.message+" => getPoolTick failed");
  }
}

async function getPoolPrice(tokenA,tokenB,poolFee)
{
  try {
    let p = await getPool(tokenA,tokenB,poolFee);
    console.log("Pool=",p);
    let pc = await getPoolContract(p);
    console.log("mint4");
    //console.log("PoolContract=",pc);
    let s0 = await slot0(pc);
    console.log("s0=",s0);
    let ts = parseInt(await tickSpacing(pc));
    console.log("ts=",ts);
    let tick = parseInt(s0[1]);
    console.log("tick=",tick);
    let price = getTickPrice(tick);
    return price;
  } catch (e) {
    console.log(e.message);
    throw new Error(e.message+" => getPoolPrice failed");
  }
}

async function mint(contract,walletAddress,lowerNut,value,vsym,ssym,tries=0,gp=false)
{
  try {
    let calls = [];
    console.log("mint1 lowerNut",lowerNut);
    let c = await erc.getContract(maps.addressMap.get(vsym));
    await erc.approve(c,walletAddress,NF_POSITION_MANAGER_ADDRESS,1);
    console.log("mint2");
    c = await erc.getContract(maps.addressMap.get(ssym));
    await erc.approve(c,walletAddress,NF_POSITION_MANAGER_ADDRESS,1);
    console.log("mint3");
    let p = await getPool(WETH_TOKEN,OP_TOKEN,POOL_FEE);
    console.log("Pool=",p);
    let pc = await getPoolContract(p);
    console.log("mint4");
    //console.log("PoolContract=",pc);
    let s0 = await slot0(pc);
    console.log("s0=",s0);
    let ts = parseInt(await tickSpacing(pc));
    console.log("ts=",ts);
    let tick = parseInt(s0[1]);
    console.log("tick=",tick);
    let upperNut = lowerNut + SPAN;
console.log("lowerNUT=",lowerNut,"upperNUT=",upperNut);
    let price = getTickPrice(tick);
    let {vr,sr} = calculateAmounts(tick,lowerNut,upperNut,value);
console.log("vr=",vr,"sr=",sr);
    let vin = BigNumber(Math.floor(sr*value*1000000)).mult(BigNumber(10).pow(18)).div(Math.floor(price*1000000)).toString();
    let sin = Math.floor(vr*value*1000000).toString();
    let vusd = parseInt(BigNumber(vin).mult(Math.floor(price*1000000)).div(BigNumber(10).pow(18)).toString())/1000000;
    let susd = sin/1000000;
    vin = BigNumber(Math.floor(vusd/maps.priceMap.get(vsym)*1000000)).mult(BigNumber(10).pow(18)).div(1000000).toString();
    sin = BigNumber(Math.floor(susd/maps.priceMap.get(ssym)*1000000)).mult(BigNumber(10).pow(18)).div(1000000).toString();
console.log("vin=",vin, vusd,"sin=",sin,susd);
    console.log("mint",vin,vusd,sin,susd);

    let t0 = maps.addressMap.get("WETH");
    let t1 = maps.addressMap.get("OP");
    if (web3.chain == "poly")
    {
      //let ln=Math.abs(lowerNut);
      //let un = Math.abs(upperNut);
      //lowerNut = un;
      //upperNut = ln;
      let t2 = t0;
      t0 = t1;
      t1 = t2;
      let in2 = vin;
      vin = sin;
      sin=in2;
    }
    let params = [
      t0,
      t1,
      POOL_FEE,
      lowerNut,
      upperNut,
      sin,
      vin,
      0,
      0,
      walletAddress,
      Math.floor(Date.now() / 1000) + 60 * 20,
    ];

    let mintEncode = await contract.methods
      .mint(params)
      .encodeABI();
    let refundEncode = await contract.methods
      .refundETH()
      .encodeABI();

    let multiParams = {
      from: walletAddress,
      value: 0,
      gas: 800000,
      nonce: await web3.obj.eth.getTransactionCount(walletAddress)
    };
    if (web3.chain == "poly")
      multiParams.maxPriorityFeePerGas = await gas.getGas();
    else if (web3.chain == "op")
    {
      //multiParams.type = 2;
      multiParams.maxPriorityFeePerGas = await gasOp.getMaxPriorityFee();
      //let alc = web.getEthersAlchemy("op");
      //let gasPrice = await alc.core.getGasPrice();
      //multiParams.maxFeePerGas = gasPrice.toString();
    }

    console.log("multiParams",multiParams);
    calls.push(mintEncode);
    calls.push(refundEncode);
console.log("calling mint transaction");
    let tx = await contract.methods
      .multicall(calls)
      .send(multiParams);
    console.log("tx=",tx);
    console.log("CONFIRMING TRANSACTION");
    await confirmTransaction(tx.transactionHash);
  } catch (e) {
    console.log(e.message);
    tries++;
/*
    if (tries < MAX_TRIES)
    {   
      let gp = utils.parseGas(e.message);
      gp = Math.floor((1+0.5*tries) * gp);
      await mint(contract,walletAddress,lowerNut,value,tries,gp);
      return; 
    }
*/
    throw new Error(e.message+" => mint failed");
  }
}

function nearestTick(tick)
{
  let nut = univ3sdk.nearestUsableTick(tick,TICK_SPACING);
  return nut;
}

function findTicks(p)
{
  let tick = getTickFromPrice(p);
  let ts = TICK_SPACING;
  let nut = univ3sdk.nearestUsableTick(tick,ts);
console.log("findTicks nut", nut);
  let lowerNut = nut - HALF_SPAN;
  let upperNut = nut + HALF_SPAN;
console.log("findTicks() lowerNut=",lowerNut,"upperNut=",
    upperNut,"tick=",tick," SPAN=",SPAN," HALF_SPAN=",HALF_SPAN);
  return {lowerNut,upperNut};
}

async function increaseLiquidity(wname,c,walletAddress,tokenid,value,vsym,ssym,tries=0,gp=false)
{
  try {
console.log("increaseLiquidity params=",wname,walletAddress,tokenid,value,vsym,ssym);
    let calls=[];

    let p = await getPool(WETH_TOKEN,OP_TOKEN,POOL_FEE);
    console.log("Pool=",p);
    let pc = await getPoolContract(p);
    //console.log("PoolContract=",pc);
    let liq = parseInt(await liquidity(pc));
    let s0 = await slot0(pc);
    let ts = parseInt(await tickSpacing(pc));
console.log("step1");
    let posInfo = await positions(c,tokenid);
console.log("step2");
    let lowerNut = posInfo.tickLower;
    let upperNut = posInfo.tickUpper;
    let tick = parseInt(s0[1]);
    if (tick < lowerNut)
      tick = lowerNut;
    else if (tick > upperNut)
      tick = upperNut;
    let price = getTickPrice(tick);
    let {vr,sr} = calculateAmounts(tick,lowerNut,upperNut,value);
console.log("vr=",vr,"sr=",sr);
/*
    let vin = BigNumber(Math.floor(vr*value*1000000)).mult(BigNumber(10).pow(18)).div(Math.floor(price*1000000)).toString();
    let sin = Math.floor(sr*value*1000000).toString();
console.log("vin=",vin, parseInt(BigNumber(vin).mult(Math.floor(price*1000000)).div(BigNumber(10).pow(18)).toString())/1000000,"sin=",sin,sin/1000000); 
*/
    let vin = BigNumber(Math.floor(sr*value*1000000)).mult(BigNumber(10).pow(18)).div(Math.floor(price*1000000)).toString();
    let sin = Math.floor(vr*value*1000000).toString();
    let vusd = parseInt(BigNumber(vin).mult(Math.floor(price*1000000)).div(BigNumber(10).pow(18)).toString())/1000000;
    let susd = sin/1000000;
    vin = BigNumber(Math.floor(vusd/maps.priceMap.get(vsym)*1000000)).mult(BigNumber(10).pow(18)).div(1000000).toString();
    sin = BigNumber(Math.floor(susd/maps.priceMap.get(ssym)*1000000)).mult(BigNumber(10).pow(18)).div(1000000).toString();
    let params; // tokenid, amount0Desired, amount1Desired, amount0Min, amount1Min, deadline
    if (web3.chain == "poly")
    {
      let in2 = vin;
      vin = sin;
      sin=in2;
    }
console.log("pos0=",vin,"pos1=",sin);
let test0 = parseInt(vin)/1000000;
let test1 = parseInt(BigNumber(sin).mult(Math.floor(price * 1000000)).div(BigNumber(10).pow(18)).toString())/1000000;
let test = test0+test1;
console.log("Amount=",test,test0,test1);
    params = [tokenid,sin,vin,0,0,Math.floor(Date.now() / 1000) + 60 * 20];
    let multiParams = {
      from: walletAddress,
      value: 0,
      gas: 800000,
      nonce: await web3.obj.eth.getTransactionCount(walletAddress)
    };
    if (web3.chain == "poly")
      multiParams.maxPriorityFeePerGas = await gas.getGas();
  console.log("increaseLiquidity params=",params);
    unic.removeUniswapId(wname,lookupPosition(tokenid));
    let increaseEncode = await c.methods
      .increaseLiquidity(params)
      .encodeABI();
    let refundEncode = await c.methods
      .refundETH()
      .encodeABI();
    calls.push(increaseEncode);
    calls.push(refundEncode);
    let tx = await c.methods
      .multicall(calls)
      .send(multiParams);
    console.log("tx=",tx);
    console.log("CONFIRMING TRANSACTION");
    await confirmTransaction(tx.transactionHash);
  } catch (e) {
    tries++;
    if (tries < MAX_TRIES)
    {   
      let gp = utils.parseGas(e.message);
      gp = Math.floor((1+0.5*tries) * gp);
      await increaseLiquidity(wname,c,walletAddress,tokenid,value,vsym,ssym,tries,gp);
      return; 
    }
    console.log(e.message);
    throw new Error(e.message+" => increaseLiquidity and refund failed");
  }
}

async function collect(wname,c,walletAddress,tokenid)
{
  try {
    let calls=[];
    let info = await positions(c,tokenid);
    let m0=MAX_ETH_IN;
    let m1=MAX_USD_IN;
    if (web3.chain == "poly")
    {
      m0=MAX_USD_IN;
      m1=MAX_ETH_IN;
    }
    let cparams = {
        tokenid: tokenid,
        recipient: walletAddress,
        amount0Max: m0,
        amount1Max: m1,
      };
    cparams = [tokenid,walletAddress,m0,m1];
console.log("rl 1");
    let collectEncode = await c.methods
      .collect(cparams)
      .encodeABI();
    calls.push(collectEncode);
console.log("rl 2");

    let multiParams = {
      from: walletAddress,
      value: 0,
      gas: 800000,
      nonce: await web3.obj.eth.getTransactionCount(walletAddress)
    };
    if (web3.chain == "poly")
      multiParams.maxPriorityFeePerGas = await gas.getGas();

    await c.methods
      .multicall(calls)
      .send(multiParams);
console.log("rl 3");
  } catch (e) {
    console.log(e.message);
    throw new Error(e.message+" => collect failed");
  }
}

async function removePositions(wname,walletAddress)
{
  try {
    console.log("removePositions()");
    init();
    let c = await getContract();
    await getTokenIds(wname,c,walletAddress);
    for( let[key,value] of liquidityMap) {
        console.log(key,value);
        let pid = lookupLowTick(key);
        console.log("Checking pid=",pid);
        if (BigNumber(value).gt(0))
        {
          console.log("Removing pid=",pid);
          await removeLiquidity(wname,c,walletAddress,pid);
          //unic.saveFile("update",wname,"Removing liquidity");
        }
    };
  } catch (e) {
    console.log(e.message);
    throw new Error(e.message+" => removePositions failed");
  }
}

let MAX_TRIES = 3;

async function removeLiquidity(wname,c,walletAddress,tokenid,tries=0,gp=false)
{
  try {
console.log("removeLiquidity", tokenid);
    let calls=[];
    let info = await positions(c,tokenid);
    let params = {
        tokenid: tokenid,
        liquidity: info.liquidity,
        amount0Min: 0,
        amount1Min: 0,
        deadline: Math.floor(Date.now() / 1000) + 60 * 20
      };
    params = [tokenid,info.liquidity,0,0,Math.floor(Date.now() / 1000) + 60 * 20];
  console.log("removeLiquidity params=",params);
    unic.removeUniswapId(wname,lookupPosition(tokenid));
    unic.removeTagId("unifees",tokenid);
    let removeEncode = await c.methods
      .decreaseLiquidity(params)
      .encodeABI();
    let cparams = {
        tokenid: tokenid,
        recipient: walletAddress,
        amount0Max: MAX_ETH_IN,
        amount1Max: MAX_ETH_IN,
      };
    let m0=MAX_ETH_IN;
    let m1=MAX_ETH_IN;
    if (web3.chain == "poly")
    {
      m0=MAX_ETH_IN;
      m1=MAX_ETH_IN;
    }
    cparams = [tokenid,walletAddress,m0,m1];
console.log("rl 1");
    let collectEncode = await c.methods
      .collect(cparams)
      .encodeABI();
    calls.push(removeEncode);
    calls.push(collectEncode);
console.log("rl 2");

    let multiParams = {
      from: walletAddress,
      value: 0,
      gas: 800000,
      nonce: await web3.obj.eth.getTransactionCount(walletAddress)
    };
    if (gp)
    {
      multiParams.gasPrice = gp;
    }
    if (web3.chain == "poly")
      multiParams.maxPriorityFeePerGas = await gas.getGas();

console.log("web3=",web3);
    let tx = await c.methods
      .multicall(calls)
      .send(multiParams);
console.log("rl 3");
    console.log("CONFIRMING TRANSACTION");
    await confirmTransaction(tx.transactionHash);
  } catch (e) {
    console.log(e.message);
    /*
      if (e.message.search("Returned error: gas price too low:")>=0)
      {
        tries++;
        if (tries < MAX_TRIES)
        {
          let gp = utils.parseGas(e.message);
          gp = Math.floor((1+0.5*tries) * gp);
          await removeLiquidity(wname,c,walletAddress,tokenid,tries,gp);
          return;
        }
      }
    */
    throw new Error(e.message+" => removeLiquidity() and collect failed");
  }
}

async function removePositions(wname,walletAddress)
{
  try {
    console.log("removePositions()");
    init();
    let c = await getContract();
    await getTokenIds(wname,c,walletAddress);
    for( let[key,value] of liquidityMap) {
        console.log(key,value);
        let pid = lookupLowTick(key);
        console.log("Checking pid=",pid);
        if (BigNumber(value).gt(0))
        {
          console.log("Removing pid=",pid);
          await removeLiquidity(wname,c,walletAddress,pid);
          //unic.saveFile("update",wname,"Removing liquidity");
        }
    };
  } catch (e) {
    console.log(e.message);
    throw new Error(e.message+" => removePositions failed");
  }
}

function getInitPosition(tl,liq,vsym,ssym)
{
  //SPAN = 180;
  //HALF_SPAN=90;
  let plower = getTickPrice(tl);
  let pupper = getTickPrice(tl+SPAN);
  const p = getTickPrice(tl+HALF_SPAN);
  if (plower > pupper)
  {
    let ptmp = plower;
    plower = pupper;
    pupper = ptmp;
  }
  let vamt18 = BigNumber(liq).mult(Math.floor((Math.sqrt(pupper) - Math.sqrt(p))/Math.sqrt(p)/Math.sqrt(pupper)*1000000)).toString();
console.log("vamt18",vamt18);
  let vamt = parseInt(BigNumber(vamt18).div(BigNumber(10).pow(18)).toString())/1000000;
console.log("vamt",vamt);
  let vusd = parseInt(BigNumber(vamt18).div(BigNumber(10).pow(12)).toString())*maps.priceMap.get(vsym);
console.log("vusd",vusd);
  let samt = BigNumber(liq).mult(Math.floor((Math.sqrt(p) - Math.sqrt(plower))*1000000)).div(BigNumber(10).pow(18)).toString();
console.log("samt",samt);
  samt = samt / 1000000;
console.log("samt",samt);
  console.log("getInitPosition","lower",tl,"liq",liq,"plower",plower,"pupper",pupper, "price",p,"samt",samt,"vamt",vamt);
  return {iVamt: samt, iSamt: vamt};
}

async function getPositions(wname,walletAddress,vsym,ssym)
{
  try {
console.log("Calling getPositions", wname, walletAddress);
    let c = await getContract();
    await getTokenIds(wname,c,walletAddress);
console.log("getTokenIds returning");
    let pos = [];
    if (wname != 'lance')
      return pos;
    console.log("ETH=",maps.priceMap.get("WETH"));
    console.log("OP=",maps.priceMap.get("OP"));
    let q = maps.priceMap.get(ssym)/maps.priceMap.get(vsym);
    let pc = await getContractForPool();
    //let tick = await getPoolTick(pc);
    //let q = getTickPrice(tick);
    console.log("liquidityMap=",liquidityMap,"q=",q);
    for( let[key,value] of liquidityMap) {
console.log("PAIR:",key,value);
        let p = q;
        let tl = parseInt(key);
        let pid = lookupLowTick(key);
        let plower = getTickPrice(tl);
        let pupper = getTickPrice(tl + SPAN);
        if (plower > pupper)
        {
          let ptmp = plower;
          plower = pupper;
          pupper = ptmp;
        }

        let inRange = true;
        if (p < plower) 
        {
          p = plower;
          inRange = false;
        }
        else if (p > pupper) 
        {
          p = pupper;
          inRange = false;
        }
        let liq = value;
        let samt2 = BigNumber(liq).mult(Math.floor(1000000*(Math.sqrt(pupper) - Math.sqrt(p))/Math.sqrt(p)/Math.sqrt(pupper))).toString();
        let samt = Math.floor(BigNumber(samt2).div(BigNumber(10).pow(18)).toString())/1000000;
        let susd = samt*maps.priceMap.get(ssym);
        //let vamt2 = BigNumber(liq).mult(Math.floor(1000000*(Math.sqrt(p) - Math.sqrt(plower))/Math.sqrt(p)/Math.sqrt(plower))).toString();
        let vamt2 = BigNumber(liq).mult(Math.floor(1000000*(Math.sqrt(p) - Math.sqrt(plower)))).toString();
        let vamt = Math.floor(BigNumber(vamt2).div(BigNumber(10).pow(18)).toString())/1000000;
        let vusd = vamt*maps.priceMap.get(vsym);
        console.log("lower",tl,"liq",liq,"plower",plower,"pupper",pupper, "price",p,susd,vusd);
        if (!BigNumber(liq).isZero())
        {
console.log("liq=",liq,pupper,plower,p,"samt2 = ",samt2,"samt=",samt,"susd=",susd);
console.log("vamt2 = ",vamt2,"vamt=",vamt,"vusd=",vusd);
        }
        var newpos = [
          { id: pid, chain: web3.chain, symbol: vsym, decimals: 18, inRange: inRange, tickLower: tl, plower: plower, pupper: pupper, liquidity: liq, amount: vamt, quote: q, usd: vusd },
          { id: pid, chain: web3.chain, symbol: ssym, decimals: 6, inRange: inRange, tickLower: tl, plower: plower, pupper: pupper, liquidity: liq, amount: samt, quote: 1, usd: susd },
          ];
        pos = pos.concat(newpos);
        if (BigNumber(liq).gt(0))
        {
console.log("here1");
          let {f0,f1} = await unifees.getFees(walletAddress,pid);
console.log("fees f0 f1",f0,f1);
console.log("here2");
          newpos = [
            { id: "fees-"+pid, chain: web3.chain, symbol: 'OP', amount: f0, usd: f0 * maps.priceMap.get("OP") },
            { id: "fees-"+pid, chain: web3.chain, symbol: 'WETH', amount: f1, usd: f1 * maps.priceMap.get("WETH") }
          ];
          pos = pos.concat(newpos);
        }
console.log("getPostions() PAIR", pos);
      };
console.log("UNI V3 positions=",pos);
    return pos;
  } catch (e) {
    console.log(e.message);
    throw new Error(e.message+" => univ3.getPositions() failed");
  }
}

function getSpanFromCenter(tickLower,q)
{
  let tickCenter = tickLower+HALF_SPAN;
  let currentTick = getTickFromPrice(q);
  return currentTick - tickCenter;
}

/*
async function main()
{
  let wallet = await wall.init("lance","op");


  let q = await quote.oneFastQuote(ETH_ADDRESS,web.chain);
  await getPositions("lance",wallet.address);
  //console.log("liquidityMap",liquidityMap);
  //console.log("lowTickMap",lowTickMap);

  let c = await getContract();
  //await mint(c,wallet.address,10);
  //await removeLiquidity(wname,c,wallet.address,344383);
  await increaseLiquidity(wname,c,wallet.address,344939,10);

}

main();
*/

// TO DO
// add init() that inserts tokens into price map
// add maps to store pool info: tick spacing, pool name, tokens in pool, etc.
// change iterator on pool to use pid since that is unique across any pool type, lower tick number is not
module.exports = Object.assign({
  init,
  getInitPosition,
  getSpanFromCenter,
  collect,
  getPositions,getContract,mint,increaseLiquidity,removeLiquidity,
  getTickPrice,getTickFromPrice,nearestTick,findTicks,
  lookupLiquidity,
  lookupLowTick,
  lookupPosition,
  getContract,
  getCurrentPrice,
  getEthPrice,
  slot0, ticks,
  feeGrowthGlobal0X128,
  feeGrowthGlobal1X128,
  positions,
  getPoolContract,
  getContractForPool,
  getPoolTick,
  removePositions
});

