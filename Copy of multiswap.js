...
console.log("checking pair",ethPairs[i]);
      if (!hasChain(port,ethPairs[i].chain))
        continue;
console.log("pair ok");
if (ethPairs[i].fusion) continue; //xxx remove fusion for now
...
//console.log("quoteEthSwap wamt=",wamt,ethPairs[i].chain);
      ...
      {
        console.log("6",fromSym,maps.addressMap.get(fromSym),toSym,maps.addressMap.get(toSym));
        try {
          q = await inch.getQuote(maps.addressMap.get(fromSym),maps.addressMap.get(toSym),fromAmt,wallet.address,ethPairs[i].chain,opt,ratio);
        } catch (e) {
