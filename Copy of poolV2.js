...
console.log("SELLING vamt=",vamt);
    ...
    let apy =  Math.floor(((1+(pos.netVusd+pos.netSusd)/port.uniswapV2.pair.value) ** ((365*24*3600)/(pos.now-pos.starttime)) * 100-100)*10)/10;
...
console.log("pos=",newpos);
    ...
    {
      let wc = utils.checkWallet(newpos.positions,"WETH",port.uniswapV2.chain);
...
console.log("u=",u,-parseFloat(newpos.netVusd));
    ...
    if (parseFloat(newpos.netVusd) > port.snapshot.threshold * port.uniswapV2.pair.optRatio)
    {
      let wc = utils.checkWallet(newpos.positions,"WETH",port.uniswapV2.chain);
      if (wc > parseFloat(newpos.netVamt))
      {
    ...
    else if (parseFloat(newpos.netVusd) < -port.snapshot.threshold * port.uniswapV2.pair.optRatio)
    {
console.log("need to sell usdc to buy weth");
      let u = utils.checkWallet(newpos.positions,"USDC",port.uniswapV2.chain);
console.log("u=",u,-parseFloat(newpos.netVusd));
...
console.log("INCLUDELIST=", includelist);
    ...
    npos = await univ2.getPositions(port.uniswapV2.wname,port.uniswapV2.walletAddress,q);
    //console.log(npos);
    pos = pos.concat(npos);
    //console.log(pos);
    let susd=0;
...
console.log("newpos=",newpos);
    ...
    let apy =  Math.floor(((1+(newpos.netVusd+newpos.netSusd)/port.uniswapV2.pair.value) ** ((365*24*3600)/(newpos.now-newpos.starttime)) * 100-100)*10)/10;
    let body="";
