...
async function swap(fromToken, toToken, amount, walletAddress, retries=0, gp=false) {
  try {
    ...
    {
      quoteUrl = `https://api.1inch.dev/swap/v5.2/43114?fromTokenAddress=${fromToken}&toTokenAddress=${toToken}&amount=${amount}&fromAddress=${walletAddress}&slippage=`+slippage;
      broadcastUrl = 'https://api.1inch.dev/tx-gateway/v1.1/43114/broadcast';
    }
    const config = {
      headers:{
...
//console.log("swap quoteUrl=",quoteUrl,config);
//exit();
    let response;
    try {
      response = await axios.get(quoteUrl,config);
    } catch (e)
    {
...
console.log("slippage=",slippage);
      const baseUrl = 'https://api.1inch.dev/swap/v5.2/'+cid;
      quoteUrl = baseUrl+'?fromTokenAddress='+fromToken+'&toTokenAddress='+toToken+'&amount='+amount+'&fromAddress='+walletAddress+'&slippage='+slippage+protocols;
      console.log("getQuote() quoteUrl=",quoteUrl+" ratio="+ratio+" ch="+ch, "config=",config);
...
console.log("swapAmount =", swapAmount);
  ...
  const result = { toAmount: toAmount, amount: swapAmount, decimals: toDecimals, symbol: toSymbol, quoteData: response, quoteUrl: quoteUrl};
  return result;
}
  ...
  maxSlip)  // slippage increases linearly from min to max over seconds
{
  ...
  while (time < startTime + seconds) {
    try {
      ...
      try {
        //console.log("getSwapQuote() quoteUrl=",quoteUrl,config);
        response = await axios.get(quoteUrl,config);
      } catch (e) {
        console.log("Error in axios.get of quote:",e.message,quoteUrl);
        if (e.message.search("status code 400")>=0)
...
async function swapWithQuote(response, fromToken, amount, walletAddress) {
  try {
    ...
    console.log("swapWithQuote",response,fromToken,amount,walletAddress);
    let data;
    if (response.data) {
      data = response.data;
...
console.log("swapWithQuote sending Transaction", data.tx, web3.chain);
    ...
    throw new Error(e.message + " => 1inch.swapWithQuote() failed");
  }
}
