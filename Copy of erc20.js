...
async function initMaps(tokenAddresses)
{
  try {
    let ch = web3.chain;
    console.log("erc20.initMaps()",tokenAddresses,tokenAddresses.length,ch);
    for (let i=0;i<tokenAddresses.length;i++)
    {
      ...
      {
console.log("i1",tokenAddresses[i]);
...
console.log("i4");
      ...
      else
      {
        ...
        if (ch == "avax")
          ...
          throw new Error("Unknown native token in initMaps()");
...
async function getContract(erc) {
  try {
    ...
    {
      throw Error("trying to get contract for native token => getContract failed");
    }
    const cmap = contractMap.get(erc);
    if (cmap && false)  // disable contract map - maybe this is caching the result xxx
    {
...
console.log("finished reading tag o=",o);
  if (o)
  {
console.log("returning", o["decimals"]);
    return o["decimals"];
  }
...
async function balanceOf(ercContract, walletAddress,retries=0) {
  //console.log("walletAddress", walletAddress,ercContract());
...
async function approve(tokenContract, walletAddress, approveAddress, amount,retries=0) {
  ...
  if (BigNumber(allow).lt(amount))
  {
    ...
    const tx = await tokenContract.methods
      .approve(approveAddress, "115792089237316195423570985008687907853269984665640564039457584007913129639935")
      .send(params)
      .catch(function (e) {
        if (retries < MAX_RETRIES)
        {
          if (utils.shouldRetry(e.message))
          {
            retries++;
            //await utils.sleep(retries);
            return approve(tokenContract, walletAddress, approveAddress, amount,retries);
          }
        }
        console.log(e.message);
