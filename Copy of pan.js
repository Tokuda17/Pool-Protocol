...
const wchefAddress = "0xa67CF61b0b9BC39c6df04095A118e53BFb9303c7";
const chefAddress = "0x1f806f7C8dED893fd3caE279191ad7Aa3798E928";
const spellAddress = "0x966bbec3ac35452133B5c236b4139C07b1e2c9b1";
...
const AVAX_USDTe_ADDRESS= "0xe28984e1EE8D431346D32BeC9Ec800Efb643eef4";

//*********************************************************************
// Pangolin contracts 
//*********************************************************************

async function getWchefContract() {
  const chefContract = await new web3.obj.eth.Contract(wchefABI, wchefAddress);
  return chefContract;
}
...
//*********************************************************************
// Pangolin Chef contracts and methods
//*********************************************************************

async function getChefContract() {
  const chefContract = await new web3.obj.eth.Contract(chefABI, chefAddress);
  return chefContract;
}

async function userInfo(c,walletAddress,pid) {
  const tx = await c.methods
    .userInfo(pid,walletAddress)
    .call({ from: walletAddress })
    .catch(function (e) {
      console.log(e.message);
      throw new Error(e.message+" => userInfo() failed");
    });
  return tx.amount;    
}
...
async function initMaps()
{
  ...
  maps.addressMap.set("AVAX-USDT.e", AVAX_USDTe_ADDRESS);
  maps.symbolMap.set(AVAX_USDTe_ADDRESS,"AVAX-USDT.e");
  maps.decimalsMap.set("AVAX-USDT.e", 18);
...
async function vaultPendingReward(pid, walletAddress)
{
  ...
  if (pid == 113)
    ...
    pname = "AVAX-USDT.e";
  let amt = parseInt(BigNumber(tx).div(BigNumber(10).pow(12)).toString())/1000000;
  let usd = amt * maps.priceMap.get("PNG");
  let reward = { id: pname, symbol: "PNG", amount: tx, decimals: 18, amt: amt, usd: usd};
  rewards.push(reward);
...
async function vaultHarvest(pid,walletAddress)
{
  ...
  const tx = await chefContract.methods
    ...
    .send({ from: walletAddress, gas: 1100000, nonce: await web3.obj.eth.getTransactionCount(walletAddress) })
    .catch(function (e) {
      console.log(e.message);
      throw new Error(e.message+" => vaultHarvest() failed");
    });
}
...
try {
  ...
  pid = 55;
  amt = await userInfo(c,walletAddress,pid);
  console.log("amt=",amt);
  let poolTokens = await getPoolTokens("AVAX-USDC",
    maps.addressMap.get("AVAX-USDC"),amt);
  lpPos = poolTokens.positions;
  pos = pos.concat(lpPos);
  pos2 = await vaultPendingReward(pid,walletAddress);
  if (pos2 && pos2[0] && BigNumber(pos2[0].amount).gt(BigNumber(HARVEST_THRESHOLD).mult(BigNumber(10).pow(18))))
  {
  ...
  pid = 7;
  amt = await userInfo(c,walletAddress,pid);
  poolTokens = await getPoolTokens("AVAX-USDT.e",
    maps.addressMap.get("AVAX-USDT.e"),amt);
  lpPos = poolTokens.positions;
  pos = pos.concat(lpPos);
  pos2 = await vaultPendingReward(pid,walletAddress);
  if (pos2 && pos2[0] && BigNumber(pos2[0].amount).gt(BigNumber(4000).mult(BigNumber(10).pow(18))))
  {
    await vaultHarvest(pid,walletAddress);
  }
  else
  ...
  pid = 9;
  amt = await userInfo(c,walletAddress,pid);
  poolTokens = await getPoolTokens("AVAX-USDC.e",
    maps.addressMap.get("AVAX-USDC.e"),amt);
  lpPos = poolTokens.positions;
  pos = pos.concat(lpPos);
  pos2 = await vaultPendingReward(pid,walletAddress);
  if (pos2 && pos2[0] && BigNumber(pos2[0].amount).gt(BigNumber(4000).mult(BigNumber(10).pow(18))))
  ...
  pid = 113;
  amt = await userInfo(c,walletAddress,pid);
  poolTokens = await getPoolTokens("AVAX-USDT",
    maps.addressMap.get("AVAX-USDT"),amt);
  lpPos = poolTokens.positions;
  pos = pos.concat(lpPos);
  pos2 = await vaultPendingReward(pid,walletAddress);
  if (pos2 && pos2[0] && BigNumber(pos2[0].amount).gt(BigNumber(4000).mult(BigNumber(10).pow(18))))
  ...
  console.log(e.message);
  throw new Error(e.message+" => pan.getPositions() failed");
}
}
...
async function getPoolTokens(id, poolAddress, tokens) {
  try {
    ...
    const totalCheck = await totalSupply(poolContract);
    if (total != totalCheck)
      return getPoolTokens(id,poolAddress,tokens);
    ...
    let price;
    let a0 = parseInt(BigNumber(amt0).div(BigNumber(10).pow(d0-6)).toString())/1000000;
    let a1 = parseInt(BigNumber(amt1).div(BigNumber(10).pow(d1-6)).toString())/1000000;
    let usd0 = a0*maps.priceMap.get(sym0);
    let usd1 = a1*maps.priceMap.get(sym1);
    if (isStablecoin(sym0))
    {
      price = parseInt(BigNumber(res._reserve0).mult(BigNumber(10).pow(d1)).mult(1000000).div(res._reserve1).div(BigNumber(10).pow(d0)).toString())/1000000;
    }
    else
    {
      price = parseInt(BigNumber(res._reserve1).mult(BigNumber(10).pow(d0)).mult(1000000).div(res._reserve0).div(BigNumber(10).pow(d1)).toString())/1000000;
    }
    const collateral = {
